...
If Not IsEmpty(Request.QueryString("EmployeeID")) Then
	strWhere = " WHERE Left(CardTypeSub,3) = 'NAB' AND EmployeeID = '" & Request.QueryString("EmployeeID") & "'"
Else
	strWhere = " WHERE Left(CardTypeSub,3) = 'NAB' AND BatchNumber = 0"
End If
...
Dim strUpdateDate
	...
	strWhere = " WHERE Left(CardTypeSub,3) = 'NAB' AND BatchNumber = 0 AND [Status] = 'Added To NA'"
	
	'Description:	Loads CDMC Details onto the page called from
	objRS.Open "SELECT * FROM qryCAPSNAToNAB WITH(NOLOCK) " & strWhere,objCon
		...
		If Not objRS.EOF Then
			...
			If trim(strStatus) = "Added To NA" Then
				...
				'strStatus = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='ExportNANAB.asp?Action=CancelNA&NAToDinersID=" & objrs("NABNAID") & "&NAEID=" & objRS("EmployeeID") & "&Status=Deleted'""; title=""Click to Remove from NA File""><i class=""fa fa-times""></i> Remove</button>"
				
			ElseIf strStatus = "Deleted" Then
				strStatusDisplay = "<span class=""badge badge-pill badge-danger"">" & strStatus & "</span>"
				'strStatus = "<button type=""button"" class=""btn btn-success btn-xs"" onclick=""self.location='ExportNANAB.asp?Action=CancelNA&NAToDinersID=" & objrs("NABNAID") & "&NAEID=" & objRS("EmployeeID") & "&Status=Added To NA'""; title=""Click to Add to NA File""><i class=""fa fa-plus""></i> Add</button>"
			...
			Response.Write "<tr><td>" & objRS("NABNAID") & "</td><td>" & objRS("RecordType") & "</td><td>" & objRS("Surname") & "</td><td>" & strStatusDisplay & "</td><td>" & objRS("BatchNumber") & "</td>" & _
				"<td title=""" & objRS("DateUpdated") & """>" & strUpdateDate & "</td><td>" & objRS("UpdatedByName") & "</td><td>" & objRS("EmployeeID") & "</td></tr>"
