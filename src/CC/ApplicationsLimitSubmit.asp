...
If Not IsEmpty(Request.QueryString("ApplicationEmployeeID")) Then
	'If the User has assigned authority (logged in) as someone else, then load details of the assigned authority instead of their own
	If Session("UserIDAuthority") <> Session("UserID") Then
		Session("ApplicationEmployeeID") = Session("EmployeeID")
	Else
		'If the user is an Admin user then allow them to view applications with the applicants details/UserID
...
If IsNull(Session("ApplicationEmployeeID")) Then Session("ApplicationEmployeeID") = Session("EmployeeID")

If Not IsEmpty(Request.QueryString("AccountCMS")) Then
	Session("CMSUserApplication") = Request.QueryString("AccountCMS")
End If

'Set the Card selected as the limit change card
If Not IsEmpty(Request.QueryString("LimitChangeCardID")) Then
	Session("LimitChangeCardID") = Request.QueryString("LimitChangeCardID")
End If
...
If IsEmpty(Session("ApplicationChecks")) OR IsNull(Session("ApplicationChecks")) Then Session("ApplicationChecks") = "Overview"
...
function GetTextKey(elem) {
	//alert(elem.value);
	//const $input = document.querySelector('.my-input');
	varUserName = elem.value

	//varUserName = elem.value.replace(/[^a-zA-Z0-9]/g, '');
	varUserName = elem.value.replace(/[^a-zA-Z-. ]/g, '');
	elem.value = elem.value.replace(/[^a-zA-Z-. ]/g, '');
	
	document.getElementById("NameChange2Help").innerHTML = varUserName.length + ' characters';
...
function SelectEmp(varEmpID) {
	...
	{
	self.location = "ApplicationsLimitSubmit.asp?Action=Search&AccountCMS=" + varEmpID;
	//self.location = "ApplicationsLimitSubmit.asp?Action=Search&EIDNo=" + varEmpID + "&AccountCMS=" + document.getElementById('AccountCMS').text;
	}
}

function ChangeNameOnCard() {
//Reloads the page after the user clicks Save Name Change
	self.location = "ApplicationsLimitSubmit.asp?NameChangeSave=" + document.getElementById('NameOnCardTitle2').value + ' ' + document.getElementById('CMSUserName2').value;
...
<main class="main py-5">
      <div class="container">
<form action="ApplicationsLimitSubmit.asp?Action=SubmitApp" method="POST" id="frm" name="frm" class="inline">
<!-- Modal -->
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <label>I <span style="font-weight:bold;color:red;"><%=Session("ApplicationEmployeeID") & " - " & strApplicantUserName%></span> DECLARE that all the application details are correct.</label><br>
			...
			<label for="UserLoggedIn">Loged On User:</label>
		  ...
		  <div class="col-md-4 sidebar">
            ...
            <div class="panel panel-shadow panel-validation mb-3">
              <div class="panel-header">
                <h4><i class="fa fa-tag"></i> Limit Application Summary</h4>
                <span class="panel-subheader">CAPS Auto-check for <%=Session("ApplicationEmployeeID") & " " & strApplicantUserName%></span>
              </div>
            ...
            <div class="my-information">
              <ul class="nav nav-tabs" id="myFiTab" role="tablist">
                <li class="nav-item" role="presentation">
                  <a class="nav-link active" id="my-cards-tab" data-toggle="tab" href="#my-cards" role="tab" aria-controls="my-cards" aria-selected="true"> Limit Change Details</a> for <%=strApplicantUserName%>
                </li>
              </ul>
...
bolCompleteCheck = True
	...
	objRS.Open "SELECT [OutDinersAddress1],[OutDinersAddress2],[IsValidPostal],[EmployeeType] FROM tblCAPSCDMCHistory WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [Deleted] = 'N'",objCon
		...
		Else
			If IsNull(objRS("IsValidPostal")) Then
				...
				bolCompleteCheck = False
			Else
				If objRS("IsValidPostal") = "Yes" Then
    ...
    '               "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Contact"">Contact Details <i class=""" & strIco & """></i>" & _
    '               "<span class=""desc"">" & strMessage & "</span></a></li>"
	
	'strContact = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Contact"" class=""block-link""><i class=""" & strIco & """></i> Contact Details</a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	strContact = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Contact"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
                "<div class=""content""><span class=""title"">Contact Details</span><span class=""description"">" & strMessage & "</span></div></a>"
	
	
	'---- START Card Selection section ---
	'If Card Selection is currently selected then add this to the State
	If Session("ApplicationChecks") = "Cards" Then 
		strActive = "active" 
	Else
		strActive = ""
	End If
	
	'If no card has yet been selected for this user then prompt them, otherwise display choice
	If IsNull(Session("LimitChangeCardID")) Or Session("LimitChangeCardID") = 0 Then
		strMessage = Session("ApplicationEmployeeID") & " no card selected for Limit Change. "
		strIco = "fa fa-fw fa-times color-red"
	Else
		strMessage = Session("ApplicationEmployeeID") & " has selected a card! "
		strIco = "fa fa-fw fa-check color-green"
	End If
	
	strCards = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Cards"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
                "<div class=""content""><span class=""title"">Card Limit Changing</span><span class=""description"">" & strMessage & "</span></div></a>"
				
	'---- END Card Selection section ---
	...
	objRS.Open "SELECT [CardNumber],[Expiry],[NameOnCard] FROM tblCAPSCard WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [Status] = '00' AND [CardType] = '" & strCardType & "'",objCon

		If objRS.EOF Then
			strMessage = Session("ApplicationEmployeeID") & " does NOT have an active card to Change Limit"
			strCheckStatus = "warning"
			strIco = "fa fa-fw fa-times color-red"
			
			bolCompleteCheck = False
		Else
			strMessage = "Existing card to change limit and employee type able to hold a card"
			strCheckStatus = "complete"
			strIco = "fa fa-fw fa-check color-green"
		End If
	...
	strEmployeeType = GetEntitlement(strCardType,strEmployeeType)
	
	If strEmployeeType = "Y" Then
		'If the Entitlement has already failed the Existing Card check above then do not make it OK below
	...
	'strEntitlement = "<li class=""" & strCheckStatus & """><a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Entitlement"" onClick=""modal();"">Entitlement <i class=""" & strIco & """></i><span class=""desc"">" & strMessage & "</span></a></li>"
	
	'strEntitlement = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Entitlement"" class=""block-link""><i class=""" & strIco & """></i> Entitlement </a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	strEntitlement = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Entitlement"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
...
End If
	...
	'strCMS =  "<li class=""" & strCheckStatus & """><a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=CMS"">CMS User Account <i class=""" & strIco & """></i><span class=""desc"">" & strMessage  & "</span></a></li>" 
	'strCMS = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=CMS"" class=""block-link""><i class=""" & strIco & """></i> CMS User Account</a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	strCMS = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=CMS"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
	...
	'strName = "<li class=""" & strCheckStatus & """><a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Name"">Name <i class=""" & strIco & """></i><span class=""desc"">" & strMessage  & "</span></a></li>"
	'strName = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Name"" class=""block-link""><i class=""" & strIco & """></i> Name</a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	strName = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Name"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
	...
	'strTraining = "<li class=""" & strCheckStatus & """><a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Training"">Training <i class=""" & strIco & """></i><span class=""desc"">" & strMessage  & "</span></a></li>"
	'strTraining = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Training"" class=""block-link""><i class=""" & strIco & """></i> Training</a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	strTraining = "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Training"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div>" & _
	...
	If bolCompleteCheck = False Then
		...
		'''*****-----Temporarly commented out so that applications can be submitted for testing -- uncomment for Production ------*************
	...
	'Write the Overview/Summary for the Employee
	'Response.Write "<li class=""" & strCheckStatus & """>" & _
	'				"<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Overview"">Overview <i class=""" & strIco & """></i>" & _
    '                "<span class=""desc"">" & strMessage  & "</span>" & _
    '                 "</a></li>"
	
	'Response.Write "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Overview"" class=""block-link""><i class=""" & strIco & """></i> Overview </a><span class=""content"" style=""font-weight:normal; color:gray; font-size:12px;"">" & strMessage & "</span>"
	
	Response.Write "<a href=""ApplicationsLimitSubmit.asp?ApplicationChecks=Overview"" class=""section-link " & strActive & """ Style=""padding:10px;""><div class=""status""><i class=""" & strIco & """></i></div><div class=""content""><span class=""title"">Overview</span>" & _
                  "<span class=""description"">" & strMessage & "</span></div></a>"
				
	'Write details of all of the other checks (after the Summary/Overview)
	Response.Write	strCards & strEntitlement & strContact & strCMS & strName & strTraining
...
strHelpMessage = ""
	...
	'Check the Employee doesn't already have any contact detail issues
	objRS.Open "SELECT [EmployeeType] FROM tblCAPSCDMCHistory WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [ActiveEmployee] = 'Y'",objCon

		If objRS.EOF Then
			strMessage2 = Session("ApplicationEmployeeID") & " has no Employment Type!? "
			strColour = "red;"
			strFaFa = "times"
			...
			strAlert = "danger"
			strValid2 = "is-invalid"
		Else
			If objRS("EmployeeType") = "Not Provided" Then
			
				strMessage2 = Session("ApplicationEmployeeID") & " has Employment Type - Not Provided"
				strColour = "red;"
				strFaFa = "times"
				'strHelpMessage = "<div class=""input-group col-sm-3"" Style=""Font-style:italic; color:orange;"">" & GetSystemAdmin("SystemAdmin") & "</div>"
				strHelpMessage = GetSystemAdmin("SystemAdmin")
				...
				strValid2 = "is-invalid"
			Else
				strMessage2 = objRS("EmployeeType")
	...
	objRS.Open "SELECT [CardNumber],[Expiry],[NameOnCard] FROM tblCAPSCard WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [Status] = '00' AND [CardType] = '" & strCardType & "'",objCon
		...
		If objRS.EOF Then
			strMessage = Session("ApplicationEmployeeID") & " does NOT have an active " & strCardType & " card to Change Limit"
			strColour = "red;"
			strFaFa = "times"
			strValid = "is-invalid"
			strAlert = "danger"
		Else
			'Do not display success if there was an issue with Entitlement above
			If strAlert = "danger" Then
				strMessage = Session("ApplicationEmployeeID") & " has an existing " & strCardType & " card (select in Card Limit Changing)"
			Else
				strMessage = Session("ApplicationEmployeeID") & " has an existing " & strCardType & " card (select in Card Limit Changing)"
				strColour = "green;"
				strFaFa = "check"
	...
	'				"<li class=""breadcrumb-item active"" Style=""Background-color:" & strColour & ";color:white;font-size:16px;""><i class=""fa fa-address-card""></i> Card Entitlement for " & strApplicantUserName & "</li>" & _
	'				"</ol>"
	...
	'				"<span class=""input-group-addon"" style=""width:20%;Background-color:#2394F2;color:white;"">Employee Type</span>" & _
	'				"<input type=""text"" class=""form-control"" id=""Message"" style=""width:100%;"" value=""" & strMessage & """>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i></div>" & strHelpMessage
	
	'Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
	'				"<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""inputAddress"">Employee Type</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""inputAddress"" placeholder=""Employee Type"" value=""" & strMessage & """/></div>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " " & strValid & " "" style=""color:" & strColour  &";""></i></div><div class=""invalid-feedback"">" & strHelpMessage & "</div>"
	
	Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
					"<div class=""form-group col-sm-8""><label for=""EmployeeType"">Employee Type</label><input type=""text""  class=""form-control " & strValid2 & """ id=""EmployeeType"" placeholder=""Employee Type"" value=""" & strMessage2 & """/>" & _
					"<div class=""invalid-feedback"">" & strHelpMessage & "</div></div>"
					
	
	
	'Response.Write "<div class=""input-group col-sm-8"">" & _
	'				"<span class=""input-group-addon"" style=""width:20%;Background-color:#2394F2;color:white;"">Existing Card</span>" & _
	'				"<input type=""text"" class=""form-control"" id=""Message"" style=""width:100%;"" value=""" & strMessage & """>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i></div></form>"
					
	'Response.Write "<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""inputAddress"">Existing Card</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""inputAddress"" placeholder=""Existing Card"" value=""" & strMessage & """/></div>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i></div></form>"
	
	'Response.Write "<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""ExistingCard"">Employee Type</label><input type=""text"" style=""width:100%;"" class=""form-control " & strValid & """ id=""ExistingCard"" placeholder=""Existing Card"" value=""" & strMessage & """/></div>" & _
	'				"<div class=""invalid-feedback"">" & strHelpMessage & "</div></div>"
					
	Response.Write "<div class=""form-group col-sm-8""><label for=""ExistingCard"">Existing Card</label><input type=""text""  class=""form-control " & strValid & """ id=""ExistingCard"" placeholder=""Existing " & strCardType & " Card"" value=""" & strMessage & """/>" & _
...
strMessage = ""
	...
	'Response.write "<ol class=""breadcrumb"" Style=""Background-color:" & strColour & ";color:white;"">" & _
	'				"<li class=""breadcrumb-item active"" Style=""Background-color:" & strColour & ";color:white;font-size:16px;""><i class=""fa fa-address-card""></i> Card CMS Account for " & strApplicantUserName & "</li>" & _
	'				"</ol>"
	
	'Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
	'				"<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""inputAddress"">CMS User Name</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""inputAddress"" placeholder=""CMS User Name"" value=""" & strCMSUserName & " " & strMessage & """ title=""" & strMessage & """/></div>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i></div>"' & strHelpMessage
					
	'Response.Write "<div class=""input-group col-sm-8"">" & _
	'				"<span class=""input-group-addon"" style=""width:20%;Background-color:#2394F2;color:white;"">CMS User Name</span>" & _
	'				"<input type=""text"" class=""form-control"" id=""Message"" style=""width:100%;"" value=""" & strCMSUserName & " " & strMessage & """ title=""" & strMessage & """>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " fa-2x"" style=""color:" & strColour  &";""></i></div></form>"
					
	'Response.Write "<div class=""row"" style=""width:100%;"">" & _
	'				"<div class=""input-group col-sm-9"">" & _
	'				"<span class=""input-group-addon"" style=""width:20%;Background-color:#2394F2;color:white;"">CMS User Name</span>" & _
	'				"<input type=""text"" class=""form-control"" id=""Message"" style=""width:100%;"" value=""" & strCMSUserName & """>" & _
	'				"</div></div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " fa-2x"" style=""color:" & strColour  &";""></i></div></form>"
	
	'Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
	'				"<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""CMSUserName"">CMS User Name</label><input type=""text"" style=""width:100%;"" class=""form-control " & strValid & """ id=""CMSUserName"" placeholder=""CMS User Name"" value=""" & strMessage & """/></div>" & _
	'				"<div class=""invalid-feedback"">" & strHelpMessage & "</div></div>"
	...
	Response.Write "<div class=""row"" style=""width:100%;""><div class=""form-group col-sm-4"">" & _
				"<button type=""button"" class=""btn btn-outline-secondary btn-block"" data-toggle=""modal"" data-target=""#EmployeeModal""><i class=""fa fa-check-search""></i>Search For CMS Account</button>" & _
				"</div></div>"
...
strHelpMessage = ""
	...
	Response.Write "<div class=""row"" style=""width:100%;""><div class=""form-group col-sm-4"">" & _
				"<button type=""button"" class=""btn btn-outline-secondary btn-block"" onClick=""self.location='ApplicationsLimitSubmit.asp?NameChange=" & strNameChangeVar & "'""><i class=""fa fa-exchange-alt""></i> Change Name</button>" & _
				"</div></div>"
...
strMessage = ""
	...
	Response.write "<div class=""alert alert-" & strAlert & """ role=""alert"" style=""color:" & strColour  &";""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i> Card Training for " & strApplicantUserName & "</div>"
	
	'Response.write "<ol class=""breadcrumb"" Style=""Background-color:" & strColour & ";color:white;"">" & _
	'				"<li class=""breadcrumb-item active"" Style=""Background-color:" & strColour & ";color:white;font-size:16px;""><i class=""fa fa-address-card""></i> Card Training for " & strApplicantUserName & "</li>" & _
	'				"</ol>"
					
	'Response.Write "<li class=""" & strState & """><a href=""#"">Training <span class=""desc"">" & strMessage & "</span></a></li>"
	
	'Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
	'				"<div class=""input-group col-sm-8"">" & _
	'				"<div class=""form-group col-sm-10""><label for=""inputAddress"">Training</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""inputAddress"" placeholder=""Training"" value=""" & strMessage & """ title=""" & strMessage & """/></div>" & _
	'				"</div><div class=""input-group col-sm-1""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i></div>" & strHelpMessage
	
	Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
					"<div class=""form-group col-sm-8""><label for=""Training"">Training</label><input type=""text""  class=""form-control " & strValid & """ id=""Training"" placeholder=""Training Complete"" value=""" & strMessage & """/>" & _
					"<div class=""invalid-feedback"">" & strHelpMessage & "</div></div>"
...
Dim strDisabled
	...
	'Write the header first depending on the card and if a card is selected
	Response.write "<div class=""alert alert-" & strAlert & """ role=""alert"" style=""color:" & strColour  &";""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i> Select a Card to Change Limit </div>"
	
	'Description:	Loads Card details into page if applicable.
	objRS.Open "SELECT [CardNumber],[Expiry],[NameOnCard],[CardID],[CardType],[CardTypeSub],[CreditLimitAmount] FROM tblCAPSCard WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [Status] = '00' AND [CardType] = '" & strCardType & "'",objCon
	'objRS.Open "SELECT Top 10 [CardNumber],[Expiry],[NameOnCard],[CardID],[CardType],[CardTypeSub],[CreditLimitAmount] FROM tblCAPSCard WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [Status] = '00' and [CardTypeSub] <> 'CTS'",objCon
		...
		If Not objRS.EOF Then
			...
			Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
				"<div class=""form-group col-sm-8""><span style=""font-weight:bold;"">Card</span></div>" & _
				"<div class=""form-group col-sm-2""><span style=""font-weight:bold;"">Current Limit</span></div>" & _
				"<div class=""form-group col-sm-2""><span style=""font-weight:bold;"">New Limit</span></div>"
		Else
			...
			'Display each card as an option to select
			Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
					"<div class=""form-group col-sm-8""><label for=""Card"">No Cards for " & strApplicantUserName & "</label><input type=""text""  class=""form-control " & strValid & """ id=""Card"" placeholder=""Card Details"" value=""No Cards to Change Limit""/>" & _
					"<div class=""invalid-feedback"" style=""color:grey; font-style:italic;"">" & strHelpMessage & "</div></div>"
		End If
		...
		Do Until objRS.EOF
			...
			If IsNull(objRS("CreditLimitAmount")) or objRS("CreditLimitAmount") = "" Then
				strCurrentLimit = FormatCurrency(0,0)
			Else
				strCurrentLimit = FormatCurrency(objRS("CreditLimitAmount"),0)
			End If
			
			'If the Card is teh same as the card selected then display the selection and options
			If clng(objRS("CardID")) = clng(Session("LimitChangeCardID")) Then
				strButtonType = "success"
				strDisabled = ""
			Else
				strButtonType = "outline-info"
				strDisabled = "DISABLED"
			End If
			
			'If strNewLimit = "" Then strNewLimit = 0
			
			'Display each card as an option to select
			'Response.Write "<div class=""form-group col-sm-8""><label for=""Card" & objRS("CardID") & """>" & objRS("CardType") & " " & objRS("CardTypeSub") & " - " & objRS("NameOnCard") & "</label><input type=""text""  class=""form-control " & strValid & """ id=""Card" & objRS("CardID") & """ placeholder=""Card Details"" value=""" & objRS("CardType") & " - " & strCardNo & """/>" & _
			'		"<div class=""invalid-feedback"">" & strHelpMessage & "</div></div>"
					
			Response.Write "<div class=""form-group col-sm-8""><button type=""button"" class=""btn btn-" & strButtonType & """ onClick=""self.location='ApplicationsLimitSubmit.asp?LimitChangeCardID=" & objRS("CardID") & "'"">" & objRS("CardType") & " " & objRS("CardTypeSub") & " - " & strCardNo & " - " & objRS("NameOnCard") & "</button></div>" & _
						"<div class=""form-group col-sm-2""><input type=""text"" DISABLED class=""form-control"" id=""CardCurrentLimit" & objRS("CardID") & """ placeholder=""Current Card Limit"" value=""" & strCurrentLimit & """/></div>" & _
						"<div class=""form-group col-sm-2""><input type=""text"" " & strDisabled & " class=""form-control"" id=""CardNewLimit" & objRS("CardID") & """ placeholder=""New Card Limit"" value=""" & strNewLimit & """/></div>"
		...
		If IsNull(Session("LimitChangeCardID")) Or Session("LimitChangeCardID") = 0 Then
		Else
			Response.Write "<div class=""form-group col-sm-8""><textarea id=""LimitJustification"" name=""LimitJustification"" placeholder=""Please provide your justification for limit change here...."" class=""form-control"" rows=""10"" cols=""20""></textarea></div>"
		End If
		
	objRS.Close
	
	
	'If this application was loaded from an AE602 form then there will be the last four digits entered in the form, so display this number
	If strLastFourDigits = "" Then
	
	Else
		Response.Write "<div class=""form-group col-sm-10""><div class=""panel panel-shadow panel-validation mb-6""><div class=""panel-header""><h5><i class=""fa fa-file""></i> AE602 Form Details</h5></div>" & _ 
				"<label for=""LastFourDigits"" style=""font-style:italic; color:grey;"">Last Four Digits entered in AE602</label><input id=""LastFourDigits"" name=""LastFourDigits"" DISABLED placeholder=""Last four digits from AE602 form"" class=""form-control"" value=""" & strLastFourDigits & """>" & _ 
				"<label for=""Justification"" style=""font-style:italic; color:grey;"">Justification entered in AE602</label><textarea id=""JustificationAE602"" name=""JustificationAE602"" DISABLED placeholder=""Justification from AE602 form"" class=""form-control"" rows=""10"" cols=""20"">" & strJustificationAE602 & "</textarea></div></div>"
...
strMessage = ""
	...
	'Description:	Loads Position details into page if applicable.
	objRS.Open "SELECT [FirstName],[Surname],[PostalMessage],[IsValidPostal] FROM tblCAPSCDMCHistory WITH(NoLock) WHERE EmployeeID = '" & Session("ApplicationEmployeeID") & "' AND [ActiveEmployee] = 'Y'",objCon
	...
	Response.write "<div class=""alert alert-" & strAlert & """ role=""alert"" style=""color:" & strColour  &";""><i class=""fa fa-" & strFaFa & " "" style=""color:" & strColour  &";""></i> Contact Details for " & strApplicantUserName & " " & strMessage & " [Is Valid Postal] = " & strIsValidPostal & "</div>"
...
'Response.Write "<form class=""form-inline""><div class=""row"" style=""width:100%;"">" & _
	Response.Write				"<div class=""input-group col-sm-12"">" & _
					...
					"<label for=""Title"">Title</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""Title"" placeholder=""Title"" value=""" & strTitle & """ title=""" & strTitle & """/></div>" & _
					"<div class=""form-group col-md-5"">" & _
					"<label for=""FirstName"">FirstName <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strFirstName) & " chars</span></label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""FirstName"" placeholder=""FirstName"" value=""" & strFirstName & """ title=""" & strFirstName & """/></div>" & _
					"<div class=""form-group col-md-5"">" & _
					"<label for=""LastName"">LastName <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strLastName) & " chars</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""LastName"" placeholder=""LastName"" value=""" & strLastName & """ title=""" & strLastName & """/></div>" & _
					"</div><div class=""form-row col-12"">" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""Address1"">Address1 <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strAddress1) & " chars</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""Address1"" placeholder=""Address1"" value=""" & strAddress1 & """ title=""" & strAddress1 & """/></div>" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""Address2"">Address2 <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strAddress2) & " chars</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""Address2"" placeholder=""Address2"" value=""" & strAddress2 & """ title=""" & strAddress2 & """/></div>" & _
					"</div><div class=""form-row col-12"">" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""Address3"">Address3 <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strAddress3) & " chars</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""Address3"" placeholder=""Address3"" value=""" & strAddress3 & """ title=""" & strAddress3 & """/></div>" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""Suburb"">Suburb <span style=""font-size:12px; font-weight:bold; color:black;"">&nbsp;&nbsp;" & Len(strSuburb) & " chars</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""Suburb"" placeholder=""Suburb"" value=""" & strSuburb & """ title=""" & strSuburb & """/></div>" & _
					"</div><div class=""form-row col-12"">" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""State"">State</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""State"" placeholder=""State"" value=""" & strState & """ title=""" & strState & """/></div>" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""PostCode"">PostCode</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""PostCode"" placeholder=""PostCode"" value=""" & strPostCode & """ title=""" & strPostCode & """/></div>" & _
					"</div><div class=""form-row col-12"">" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""WorkPhone"">WorkPhone</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""WorkPhone"" placeholder=""WorkPhone"" value=""" & strWorkPhone & """ title=""" & strWorkPhone & """/></div>" & _
					"<div class=""form-group col-md-6"">" & _
					"<label for=""MobilePhone"">MobilePhone</label><input type=""text"" style=""width:100%;"" class=""form-control"" id=""MobilePhone"" placeholder=""MobilePhone"" value=""" & strMobilePhone & """ title=""" & strMobilePhone & """/></div>" & _
					"</div>" & _
...
Dim intRecord
  	...
  	With objCmd
			...
			.CommandType = 4
			.CommandText = "spCAPSCDMCToApplication"
			...
			.Parameters("EmployeeID") = Session("ApplicationEmployeeID")
