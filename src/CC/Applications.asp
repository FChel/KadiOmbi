...
Dim strEmployeeIDClose
	...
	If Not IsEmpty(Request.QueryString("ApplicationStatusName")) Then
		Session("ApplicationStatusNameView") = Request.QueryString("ApplicationStatusName")
	...
	If Not IsEmpty(Request.QueryString("ViewButton")) Then
		...
		Session("ApplicationStatusNameView") = ""
  ...
  If Not IsEmpty(Request.QueryString("EmployeeID")) Then
		strEmployeeIDClose = Request.QueryString("EmployeeID")
	End If
%>
...
function SaveStatus() {
	self.location = 'Applications.asp?SaveStatus='+frm.AppStatID.value+'&NewStat='+frm.NewStatus.options[frm.NewStatus.selectedIndex].value;
}
...
function ChangePage() {
	...
	self.location = 'Applications.asp?PageCombo=' + result;
	//alert(result);
	//document.getElementById('CardType').value=result;
...
function showAppVersion(varAppID) {
	///console.log('Showing....')
	//$('#ModalAppVersion').modal('show');
	
    //document.getElementById("ModalAppVersion").style.display = "block"
    document.getElementById("ModalAppVersion").style.display = "block"
    document.getElementById("ModalAppVersion").classList.add("show")

	LoadXML(varAppID);
}


function hideAppVersion() {
	console.log('Hiding....')
	//$('#ModalAppVersion').modal('show');
	
    //document.getElementById("ModalAppVersion").style.display = "block"
    document.getElementById("ModalAppVersion").style.display = "none"
    document.getElementById("ModalAppVersion").classList.remove("show")


}

function LoadXML(varAppID) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
     document.getElementById("ModalAppVersionMessage").innerHTML = this.responseText;
    }
  };

  xhttp.open("GET", "AJAX/GetXMLDetails.asp?ApplicationID=" + varAppID, true);
  xhttp.send();
}
...
<form action="Applications.asp?Action=Search&Link=AP" method="POST" id="frm" name="frm">
...
<!-- Start Download Excel Modal -->
	...
	<!-- Start App Version Modal -->
	<div class="modal fade" id="ModalAppVersion" tabindex="-1" role="dialog" aria-labelledby="ModalAppVersionCenterTitle" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered" role="document" >
		  	<div class="modal-content">
				<div class="modal-header">
			  		<h5 class="modal-title" id="ModalDeleteLongTitle" style="font-weight:bold;">XML Application Details</h5>
			  		<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				</div>
...
<div id='tbl-container'>
	...
	<section class="breadcrumbs py-2">
		<div class="row">
			<div class="col-md-8">
				<h2 class="text-left">All Applications <% If Session("UserView") = "User" Then Response.write " for " & Session("UserName")%> <i class="fa help-tooltip fa-question-circle" data-toggle="tooltip" title="All Card Applications. Can be filtered and searched."></i></h2>
			</div>
			<div class="col-md-4 text-right">
				...
				<button type="button" class="btn btn-primary" onClick='window.location="ApplicationsSubmit.asp?Link=AP"'><i class="fa fa-plus"></i> New Application</button>
			</div>
...
Dim strSQLExcel
	...
	'Only get the Status from the View if the drop down status (NEW) has not been selected
	If Session("ApplicationStatusNameView") = "" Or IsNull(Session("ApplicationStatusNameView")) Then
	...
	'If an Application Status has been selected then add that to the WHERE statement
	If Not IsNull(Session("ApplicationStatusNameView")) Then
		If Session("ApplicationStatusNameView") = "" Then
		Else
			strWhere = strWHERE & " AND [Status] = '" & Session("ApplicationStatusNameView")  &"'"
		End If
...
If strSearch = "" OR ISNull(strSearch) Then
	If Session("UserView") = "All" Then
		'If this screen has come from the APplication Detail screen via the Close Employee button then show the selected employee only
		If IsNull(strEmployeeIDClose) or strEmployeeIDClose= "" Then
		Else
			strWhere = strWhere & " AND EmployeeID = '" & strEmployeeIDClose & "'"
		End If
		
		strSQL = "SELECT TOP 1000 * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE [ApplicationID] > 0 " & strWhere & strSort

	Else
		strSQL = "SELECT TOP 1000 * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE EmployeeID = '" & Session("EmployeeID") & "' " & strSort
	End If
...
Else
	...
	If UCASE(Left(strSearch,2)) = "D:" Then
		...
		If IsDate(strSearchDate) Then 'DateAdd("d", -1, strSearchDate)
			...
			'strSQL = "SELECT * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE (DateSubmitted > '" & strSearchDate & "' AND DateSubmitted < " & strSearchDate & ")" & strWhere & strSort
			strSQL = "SELECT TOP 1000 * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE (DateSubmitted > '" & strDateFrom & "' AND DateSubmitted < '" & strDateTo & "')" & strWhere & strSort
			
			'strSQL = "SELECT * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE (DateSubmitted > '" & DateAdd("d", -1, strSearchDate) & "' AND DateSubmitted < '" & DateAdd("d", +1, strSearchDate) & "')" & strWhere & strSort
			'response.write strsql
		Else
			strSQL = "SELECT TOP 1000 * FROM qryCAPSApplicationsList WITH(NOLOCK) WHERE (DateSubmitted = '" & strSearchDate & "')" & strWhere & strSort
		End If
...
End If
	...
	'set sql string for excel
	strSQLExcel = Replace(strSQL,"TOP 1000 ","")
...
'Response.Write strSQL
'Response.Write "<br>"
Session("ExcelSearch") = strSQLExcel
'Response.Write Session("ExcelSearch")
...
'response.write strSQL	
	...
	Else
		...
		Response.Write "<div class=""panel panel-light mb-1""><div class=""panel-header""><h4></h4>" & _
		...
		If Right(Session("ApplicationTypeNameView"),12) = "Limit Change" Then
			strApplicationTypeNameSelected = "<th scope=""col"" style=""font-size:14px;""><a href=""Applications.asp?Sort=LimitDateFrom&Link=AP&SortType=" & strOrderType & """> Limit Start <i class=""fa fa-sort" & arrSort(9) & """></i></a></th>"
		Else
			strApplicationTypeNameSelected = ""
		End If
		
		Response.Write "<div class=""row""><div class=""col-md-12 col-lg-12"">" & _
			"<table class=""table table-compact text-left""><thead><tr>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=ApplicationID&Link=AP&SortType=" & strOrderType & """> App ID <i class=""fa fa-sort" & arrSort(1) & """></i></a></th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=EmployeeID&Link=AP&SortType=" & strOrderType & """> EID <i class=""fa fa-sort" & arrSort(2) & """></i></a></th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=Surname&Link=AP&SortType=" & strOrderType & """> Name <i class=""fa fa-sort" & arrSort(3) & """></i></a></th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=CardType&Link=AP&SortType=" & strOrderType & """> Card Type <i class=""fa fa-sort" & arrSort(4) & """></i></a></th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=ApplicationType&Link=AP&SortType=" & strOrderType & """> App Type <i class=""fa fa-sort" & arrSort(5) & """></i></a></th>" & _
			"<th scope=""col"">Address</th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=Status&Link=AP&SortType=" & strOrderType & """> Status  <i class=""fa fa-sort" & arrSort(6) & """></i></a> <i class=""fa help-tooltip fa-question-circle"" data-toggle=""tooltip"" title=""Card Process Status - the stage the application is at.""></i></th>" & _
			"<th scope=""col"" Title=""Date AE602 XML Form was LOADED into CAPS""><a href=""Applications.asp?Sort=DateSubmitted&Link=AP&SortType=" & strOrderType & """> Submitted <i class=""fa fa-sort" & arrSort(7) & """></i></a></th>" & _
			"<th scope=""col""><a href=""Applications.asp?Sort=DateReviewed&Link=AP&SortType=" & strOrderType & """> Reviewed <i class=""fa fa-sort" & arrSort(8) & """></i></a></th>" & _
	...
	'Do While Not objRS.Eof AND x < Session("PageCombo")'objRS.Pagesize
		...
		'If y <= clng(lngStartingPage)*clng(Session("PageCombo")) AND y >= (clng(lngStartingPage)*clng(Session("PageCombo")))-clng(Session("PageCombo")) Then
			...
			Case  "Received"
				strAction = "<button type=""button"" class=""btn btn-primary btn-sx"" onclick=""self.location='Applications.asp?Action=Release&Link=AP&ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-key""></i> Release</button>"
				strAction = strAction & " <button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='Applications.asp?Action=Reject&Link=AP&ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-cogs""></i> Reject</button>"
			Case "Added To CS"
			...
			Case "Added To NA"
				...
				strAction = "<button type=""button"" class=""btn btn-secondary btn-xs"" onclick=""self.location='../Admin/CAPSAdmin/ExportNANAB.asp?ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-key""></i> View NA</button>"	
				
				strStatus = "<span class=""badge badge-pill badge-info"" style=""font-size:12px;"">" & objRS("Status") & "</span>"
			Case "Submitted"
				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='Applications.asp?Action=Cancel&ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-minus-circle""></i> Cancel</button>"
			...
			Case "Rejected"
				'strAction = "Deleted - " & objRS("DateUpdated")'<button type=""button"" class=""btn btn-danger"" onclick=""self.location='Applications.asp?Action=Cancel&ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-minus-circle""></i> Deleted</button>"
				strAction = "<Span style=""font-size:12px;"">Rejected - " & dteDateUpdated & "</span>"'<button type=""button"" class=""btn btn-danger"" onclick=""self.location='Applications.asp?Action=Cancel&ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-minus-circle""></i> Deleted</button>"
				'strStatus  = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='Applications.asp?ApplicationID=" & objrs("ApplicationID") & "'"";>Cancelled By Applicant</button>"
			...
			Case  "Awaiting Review"
				...
				strStatus = "<span class=""badge badge-pill badge-warning"" style=""font-size:12px;"" data-toggle=""modal"" data-target=""#StatusModal"" data-AppID=""" & objRS("ApplicationID") & """  data-AppName=""" & objRS("FirstName") & " " & objRS("Surname") & " - " & objRS("CardType") & " " & objRS("CardTypeSub") & " Application"" onClick=""OpenSs(this);"">" & objRS("Status") & "</span>"
				
			Case  "Awaiting issue"
				strAction = "<button type=""button"" class=""btn btn-secondary btn-xs"" onclick=""self.location='../Admin/CAPSAdmin/ExportNANAB.asp?ApplicationID=" & objrs("ApplicationID") & "'"";><i class=""fa fa-key""></i> View NA</button>"	
				strStatus = "<span class=""badge badge-pill badge-info"">" & objRS("Status") & "</span>"
			Case "On Hold"
				strStatus = "<span class=""badge badge-pill badge-secondary"" style=""font-size:12px;"" data-toggle=""modal"" data-target=""#StatusModal"" data-AppID=""" & objRS("ApplicationID") & """  data-AppName=""" & objRS("FirstName") & " " & objRS("Surname") & " - " & objRS("CardType") & " " & objRS("CardTypeSub") & " Application"" onClick=""OpenSs(this);"">" & objRS("Status") & "</span>"
				strAction = "<button type=""button"" class=""btn btn-outline-" & strViewAppType & " btn-xs"" onclick=""self.location='ApplicationDetail.asp?Link=AP&ApplicationID=" & objrs("ApplicationID") & "'""; title=""" & strViewAppTitle & """><i class=""fa fa-file""></i> View App</button>"
			...
			Case "Old App Version"
				strAction = "<button type=""button"" class=""btn btn-outline-" & strViewAppType & " btn-xs"" onclick=""self.location='ApplicationDetail.asp?Link=AP&ApplicationID=" & objrs("ApplicationID") & "'""; title=""" & strViewAppTitle & """><i class=""fa fa-file""></i> View App</button>"
				strStatus = "<span class=""badge badge-pill badge-danger"" style=""font-size:12px;"" data-toggle=""modal"" data-target=""#StatusModal"" data-AppID=""" & objRS("ApplicationID") & """  data-AppName=""" & objRS("FirstName") & " " & objRS("Surname") & " - " & objRS("CardType") & " " & objRS("CardTypeSub") & " Application"" onClick=""OpenSs(this);"">" & objRS("Status") & "</span>"
			Case Else
				...
				strAction = "<button type=""button"" class=""btn btn-outline-" & strViewAppType & " btn-xs"" onclick=""self.location='ApplicationDetail.asp?Link=AP&ApplicationID=" & objrs("ApplicationID") & "'""; title=""" & strViewAppTitle & """><i class=""fa fa-file""></i> View App</button>"
				'strStatus = "<span class=""badge badge-pill badge-danger"" style=""font-size:12px;"">" & objRS("Status") & "</span>"
				strStatus = "<span class=""badge badge-pill badge-secondary"" style=""font-size:12px;"" data-toggle=""modal"" data-target=""#StatusModal"" data-AppID=""" & objRS("ApplicationID") & """  data-AppName=""" & objRS("FirstName") & " " & objRS("Surname") & " - " & objRS("CardType") & " " & objRS("CardTypeSub") & " Application"" onClick=""OpenSs(this);"">" & objRS("Status") & "</span>"
			End Select
	...
	If objRS("ApplicationType") = "AE602 XML" Then
		strAppForm = "<i title=""Old style AE602 PDF/XML form completed and emailed to Credit Cards"" class=""fa fa-file-pdf""></i>"
	ElseIf objRS("ApplicationType") = "Portal" Then
		strAppForm = "<i title=""New DCCP Portal Application"" class=""fa fa-globe""></i>"
	Else
		strAppForm = "<i title=""Service Connect Application"" class=""fa fa-link""></i>"
	End If
		
			response.write "<TR><TD><a Target=""_self"" HREF=""ApplicationDetail.asp?Link=AP&ApplicationID=" & objRS(0) & """>" & objRS(0) & " " & strAppForm &  "</a></TD><TD>" & objRS("EmployeeID") & "</a></TD>" & _
					...
					"<TD style=""font-size:13px; text-align:center;"">" & dteDateSubmitted & "</TD><TD style=""font-size:13px; text-align:center;"">" & dteDateReviewed & "</TD>" & strApplicationTypeNameSelected & _
					"<TD>" & strAction & " <button type=""button"" class=""btn btn-outline-dark btn-xs"" onclick=""LoadXML(" & objRS(0) & ");"" data-toggle=""modal"" data-target=""#ModalAppVersion"" title=""Click to view Application Version and other XML file details for " & strApplicantName & " ""><i class=""fa fa-info""></i></button>	</TD></TR>"
			
'Testing For the display of app version ---not working display modal
'<button type=""button"" class=""btn btn-outline-dark btn-xs"" onmouseover=""showAppVersion();"" ><i class=""fa fa-info""></i></button>			


			'response.write "<TR><TD style=""text-align:center;""><a Target=""_self"" HREF=""Applications.asp?ApplicationID=" & objRS(0) & """>" & objRS(0) & "</a></TD><TD>" & strAction & "</a></TD>" & _
			'		"<TD><a Target=""_self"" HREF=""Applications.asp?ApplicationID=" & objRS(0) & """>" & objRS(1) & "</a></TD><TD><a Target=""_self"" HREF=""Applications.asp?ApplicationID=" & objRS(0) & """>" & objRS(2) & "</a></TD>" & _
...
'			For x = 1 to y
'				strPages = strPages & "<a href=""Applications.asp?Link=AP&StartingPage=" & (x * 50) & """> " & x & " </a>"
'			Next
...
'	If y > 0 Then
'		Response.Write "<TR><TH colspan=""9"" style=""text-align:center;""><a href=""Applications.asp?Link=AP&Previous&StartingPage=" & lngStartingPage -50 & """>Previous Page " & strPages & " <a href=""Applications.asp?Link=AP&Previous&StartingPage=" & lngStartingPage + 50 & """> Next Page</TH></TR>"
'	End If
	...
	If lngTotalPages > 0 Then
		...
		'	strPages2 = strPages2 & "<li class=""page-item""><a class=""page-link"" href=""Applications.asp?StartingPage=" & lngTotalRecords - clng(Session("PageCombo")) & """ aria-label=""More""><span aria-hidden=""true"">&hellip;</span><span class=""sr-only"">More</span></a></li>"
		'End If
		...
		'	"</a></li></ul></nav></div></div></div>"
				...
				'"<a class=""page-link"" href=""Applications.asp?StartingPage=" & lngTotalRecords - clng(Session("PageCombo")) & """ aria-label=""Next""><span aria-hidden=""true"">&raquo;</span><span class=""sr-only"">Next</span>" & _
	End If
...
If Session("ApplicationTypeNameView") <> "" Or Session("ApplicationStatusNameView") <> ""  Then
Else
...
If Session("ApplicationTypeNameView") = "" Or IsNull(Session("ApplicationTypeNameView")) Then
	strAppText = "App Type"
Else
	strAppText = Left(Session("ApplicationTypeNameView"),10)
	arrButton(7) = "active"
End If

'Create the Application Type selection/filter
strDropDown = "<div class=""dropdown""><button class=""btn btn-outline-primary btn-sm dropdown-toggle " & arrButton(7) & """ type=""button"" id=""dropdownMenuButton"" data-toggle=""dropdown"" aria-haspopup=""true"" aria-expanded=""false"">" & strAppText & "</button>" & _
		...
		objRS.Open "SELECT [ApplicationTypeName] FROM tblCAPSApplication WITH(NOLOCK) GROUP BY [ApplicationTypeName]",objCon

		Do Until objRS.EOF
		
			strDropDown = strDropDown & "<a class=""dropdown-item"" href=""Applications.asp?ApplicationTypeName=" & objRS("ApplicationTypeName") & """>" & objRS("ApplicationTypeName") & "</a>"
...
'---New Drop down for he selection of Application Status -- May 2022
'Make the Button text the selected Application Status if one is selected
If Session("ApplicationStatusNameView") = "" Or IsNull(Session("ApplicationStatusNameView")) Then
	strAppText2 = "App Status"
Else
	strAppText2 = Left(Session("ApplicationStatusNameView"),10)
	arrButton(8) = "active"
End If

'Create the Application Type selection/filter
strDropDown2 = "<div class=""dropdown""><button class=""btn btn-outline-primary btn-sm dropdown-toggle " & arrButton(8) & """ type=""button"" id=""dropdownMenuButton"" data-toggle=""dropdown"" aria-haspopup=""true"" aria-expanded=""false"">" & strAppText2 & "</button>" & _
		"<div class=""dropdown-menu"" aria-labelledby=""dropdownMenuButton""><a class=""dropdown-item"" href=""Applications.asp?ApplicationStatusName="">All</a>"
		
		objRS.Open "SELECT [Status] FROM tblCAPSApplication WITH(NOLOCK) GROUP BY [Status] ORDER BY [Status]",objCon

		Do Until objRS.EOF
		
			strDropDown2 = strDropDown2 & "<a class=""dropdown-item"" href=""Applications.asp?ApplicationStatusName=" & objRS("Status") & """>" & objRS("Status") & "</a>"
		...
		strDropDown2 = strDropDown2 & "</div></div>"
		
		
	Response.Write "<div class=""btn-group btn-selector"" role=""group"" aria-label=""Basic example"">" & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(1) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=All';""><i class=""fa fa-folder""></i> View All</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(3) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=OnHold';""><i class=""fa fa-pause-circle""></i> On Hold</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(2) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=Review';""><i class=""fa fa-clock""></i> Awaiting Review</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(5) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=AddedToNA';""><i class=""fa fa-file""></i> Added To NA</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(4) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=AwaitingIssue';""><i class=""fa fa-truck""></i> Awaiting Issue</button>" & strDropDown & strDropDown2 & _
				"<button type=""button"" class=""btn btn-outline-primary btn-sm " & arrButton(6) & """ onClick=""self.location.href='Applications.asp?Link=AP&ViewButton=TempHold';"" Title=""Click to view Applications on TEMP HOLD""><i class=""fa fa-hand-paper""></i> </button>" & _
				"</div>"
