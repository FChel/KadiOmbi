...
function CheckAustralianDate(dateString,fieldName) 
{
	var error = 0;
	var dateStringValue = dateString.value;
	
	if(dateStringValue.length < 10){
     		alert(fieldName + " must be a date in dd/mm/yyyy format." );
     		dateString.focus();
	    	return false;
	}    	

	var year   = dateStringValue.substring(6,10);
	var month  = dateStringValue.substring(3,5);
	var day    = dateStringValue.substring(0,2);
	var slash1 = dateStringValue.substring(2,3);
	var slash2 = dateStringValue.substring(5,6);
	
	// alert ("passed date value = " +dateStringValue);
	// alert ("passed year = " +year);
	// alert ("passed month = " +month);
	// alert ("passed day = " +day);
	// alert ("slash= " +slash);
...
// *****
	...
	{
		// check leap year
		//var resultant = parseInt(year/4);
		//alert ("result = " +resultant);
	    //if (isNaN(resultant))
	    //{
		//	error = 1
	    //}
	...
	if (error == 1)
	{
        ...
        if (error == 2)
        {
      		alert(fieldName + " must be a future date" );
     		dateString.focus();
	    	return false;
		}
	else
		if (error == 3)
		{
			alert(fieldName + " must be in dd/mm/yyyy date format, e.g. 01/01/2000" );
			dateString.focus();
			return false;
		}		
	else 
...
//Returns true if the first date is less then the second date
function CheckDateRange(theFirstDate, theFirstField, theSecondDate, theSecondField) 
{
	var datestring1 = theFirstDate.value;
	var datestring2 = theSecondDate.value;
	var result = false
	
	if(CheckAustralianDate(theFirstDate, theFirstField)==false){
	    	return false;
	}    	

	if(CheckAustralianDate(theSecondDate, theSecondField)==false){
	    	return false;
	}    	

	if(parseInt(datestring1.substring(6,10)) > parseInt(datestring2.substring(6,10))){
		result = false;
	}else{
		if(parseInt(datestring1.substring(6,10)) == parseInt(datestring2.substring(6,10))){
			if(parseInt(datestring1.substring(3,5)) > parseInt(datestring2.substring(3,5))){
				result = false;
			}else{
				if(parseInt(datestring1.substring(3,5)) == parseInt(datestring2.substring(3,5))){	
					if(parseInt(datestring1.substring(0,2)) > parseInt(datestring2.substring(0,2))){
						result = false;
					}else{
						result = true;
					}				
	...
	if(result==false){
		alert(theSecondField + ' date must be greater than ' + theFirstField);
		return false;
	}else{
		return true;
	}		
