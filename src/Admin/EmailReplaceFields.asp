...
Dim strSQL
    ...
    Session("CurrentPage") = "Admin/EmailReplaceFields.asp"
...
If Request.QueryString("Action") = "Delete" Then   
    ...
    Call DeleteData(Request.QueryString("string_pattern"))
...
function SaveEmailTemplate() {
	...
	var id = document.getElementById('string_pattern').value
	self.location = "EmailReplaceFields.asp?Action=Save&string_pattern=" + id;
...
function loadDocE(cb) {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
     document.getElementById("GetEmailVariables").innerHTML = this.responseText;
    }
  };
  xhttp.open("GET", "../CC/AJAX/GetEmailVariables.asp?string_pattern=" + id, true);
  xhttp.send();
}
...
var name = cb.getAttribute('data-EmailTemplateName');
	...
	document.getElementById("ModalDeleteMessage").innerHTML = 'Do you wish to delete the selected Email Template - ' + id + '?';
...
function deleteEmailTemplate(cb) {
	...
	self.location = "EmailReplaceFields.asp?Action=Delete&string_pattern=" + id;
...
<main class="main py-4">
	<div class="container">			
		<div class="row mb-3">
			<div class="col-md-10">
					<h3 class="text-left">Email Variables </h3>
			</div>
			...
			<div class="col-md-2 text-right"></div>
				...
				<div class="row">
					<div class="col-md-3">
						<h6 class="mb-3">Select Email Variable</h6>
						<div
	...
	<!-- Start Edit Modal -->
	<form action="EmailReplaceFields.asp?Action=Save" method="POST" id="frm" name="frm" class="needs-validation" novalidate>
...
Dim strSQLSeacrhString
Dim strStatus
Dim strSelected
Dim intRecordCount


strSQL = "SELECT * FROM tblStringreplace WHERE BusinessArea = '" & Session("BusinessArea") & "'"
intRecordCount = 1
...
objRS.Open strSQL,objCon   
	...
	strSelected = "false"
		...
		If objRS("string_pattern") = Session("string_pattern") Then
			strSelected = "true"		
		End If

		If IsEmpty(Session("string_pattern")) and intRecordCount =  1 Then
				strSelected = "true"
		End If		
	
			str_string_pattern = objRS("string_pattern")
			strSQLSeacrhString = objRS("SQLSearchString")
			
		If strSelected = "true" Then			
			Response.Write "<a class=""nav-link active"" data-id=""" & intRecordCount & """ id=""v-pills-template-" & intRecordCount & "-tab"" data-toggle=""pill"" href=""#v-pills-template-" & intRecordCount & """ role=""tab"" aria-controls=""v-pills-template-1"" aria-selected=""" & strSelected & """ onClick=""loadDocE(this);"">" & str_string_pattern & "</a>"
		Else
			Response.Write "<a class=""nav-link"" id=""v-pills-template-" & intRecordCount & "-tab"" data-toggle=""pill"" href=""#v-pills-template-" & intRecordCount & """ role=""tab"" aria-controls=""v-pills-template-1"" aria-selected=""" & strSelected & """ >" & str_string_pattern & "</a>"
		End If
...
If IsEmpty(Session("string_pattern")) Then Session("string_pattern") = 0 End If
...
objRS.Open strSQL,objCon   
	...
	Do until objRS.EOF	
		...
		If objRS("string_pattern") = Session("string_pattern") Then
			strSelected = "true"			
		Else
			If intRecordCount = 1 and Session("string_pattern") = "0" Then
				strSelected = "true"
			Else
				strSelected = "false"	
			End If		
	...
	End If
			...
			Response.Write "<div class=""content-spacer"">"
				Response.Write "<div class=""row tab-content-header"">"
					  	Response.Write "<div class=""col-md-6 my-auto""><h4>" & str_string_pattern & "</h4></div>"
						Response.Write "<div class=""col-md-6 text-md-right""><button class=""btn btn-sm btn-outline-primary"" data-toggle=""modal"" data-target=""#emailModal"" data-id=""" & str_string_pattern & """ data-EmailType=""" & str_string_pattern & """ onClick=""loadDocE(this);""><i class=""fa fa-pen""></i> Edit template </button>"
						
						Response.Write "<button class=""btn btn-sm btn-outline-danger"" data-toggle=""modal"" data-target=""#ModalDelete"" data-id=""" & str_string_pattern & """ data-EmailTemplateName=""" & str_string_pattern & """ onClick=""loadDelete(this);""><i class=""fa fa-trash""></i> Delete template</button></div>"
					
						Response.Write "<div class=""col-12""><hr /></div>"
						
						  	Response.Write "<div class=""col-12 information-rows"">"
								
								Response.Write "<div class=""row""><div class=""col-lg-2 col-md-3""><b>Variable&nbsp;Name</b></div><div class=""col-lg-10 col-md-9"">" & str_string_pattern & "</div></div>"
								Response.Write "<div class=""row""><div class=""col-lg-2 col-md-3""><b>SQL Search String</b></div><div class=""col-lg-10 col-md-9"">" & strSQLSeacrhString & "</div></div>"
...
Sub SaveEmailTemplate()
	...
	intModalID = Request.Form("ModalSaveID")
  		...
  		With objCmd
			...
			.CommandType = 4
			.CommandText = "spCAPSStringReplaceSave"

			.Parameters.Append objCmd.CreateParameter("StringPattern", adVarChar, adParamInput, 50)
			.Parameters.Append objCmd.CreateParameter("StringReplacement", adVarChar, adParamInput, 50)
			.Parameters.Append objCmd.CreateParameter("SQLSearchString", adVarChar, adParamInput, 500)
			.Parameters.Append objCmd.CreateParameter("BusinessArea", adVarChar, adParamInput, 50)
						
			.Parameters("StringPattern") = Request.Form("VariableName" & intModalID & "")
			.Parameters("StringReplacement") = "@SearchString"
			.Parameters("SQLSearchString") = Request.Form("SQLSearchString" & intModalID & "")					
...
Public Sub DeleteData(ID)
		...
		objCon.Execute "DELETE tblStringreplace WHERE string_pattern= '" & ID & "'"
