...
Dim strHeaderText
	...
	If Request.QueryString("Action") = "CancelCS" Then
		...
		Call RemoveCSRecord(Request.QueryString("CSFromDinersID"),Request.QueryString("CSEID"),Request.QueryString("Status"))
...
function ChangeBatch() {
	...
	self.location='CSTransactionsNAB.asp?FileLoadID='+result	
...
function ChangePage() {
	...
	self.location = 'CSTransactionsNAB.asp?PageCombo=' + result;
	//alert(result);
	//document.getElementById('CardType').value=result;
         ...
         <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="compareModalLabel">
                  CS To NAB File Detail - <%=strCardType%>
                </h5>
...
<div class="modal fade" id="ModalSelectBatch" tabindex="-1" role="dialog" aria-labelledby="ModalSelectBatch" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalApproveTitle" style="font-weight:bold;">NAB CS File Batch Number</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div id='tbl-container'>
  <form action="CSTransactionsNAB.asp?Action=Search" method="POST" id="frm" name="frm">
	...
	<section class="breadcrumbs py-2">
		<div class="row">
			<div class="col-md-10">
				...
				<h4 class="text-left" data-toggle="modal" data-target="#ModalSelectBatch"  Title="Click to Select a CS File to View">CS File From NAB - <%="File Load ID : " & strHeaderText%>&nbsp;&nbsp;- Date Loaded : <%=Get_Batch_Date(Session("FileLoadID"))%> <button type="button" class="btn btn-primary" onClick="#" data-toggle="modal" data-target="#ModalSelectBatch"  Title="Click to Select a CS File to View"><i class="fa fa-file"></i> Select File</button></h4>
			</div>
			<div class="col-md-2">
...
If strSearch = "" OR ISNull(strSearch) Then
	...
	IF Session("FileLoadID") = "0" Then
		'strSQL = "SELECT " & strTOP & " * FROM qryCAPSCSToDinersList WITH(NOLOCK) WHERE CardType = '" & Request.QueryString("CardType") & "' AND (FileSeqNum = '' OR FileSeqNum IS NULL) AND [CSFromDinersID] > 0 " & strWhere & strSort
		strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE (FileSeqNum = '' OR FileSeqNum IS NULL) AND [CSFromDinersID] > 0 " & strWhere & strSort
		
	Else
	
		'strSQL = "SELECT " & strTOP & " * FROM qryCAPSCSToDinersList WITH(NOLOCK) WHERE CardType = '" & Request.QueryString("CardType") & "' AND (FileSeqNum = '" & Session("FileLoadID") & "') AND [CSFromDinersID] > 0 " & strWhere & strSort
		strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE (FileSeqNum = '" & Session("FileLoadID") & "') AND [CSFromDinersID] > 0 " & strWhere & strSort
...
Else
	If Session("FileLoadID") = 0 THEN
		'strSQL = "SELECT " & strTOP & " * FROM qryCAPSCSToDinersList WITH(NOLOCK) WHERE CardType = '" & Request.QueryString("CardType") & "' AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
		strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
	Else
		'strSQL = "SELECT " & strTOP & " * FROM qryCAPSCSToDinersList WITH(NOLOCK) WHERE CardType = '" & Request.QueryString("CardType") & "' AND FileSeqNum = '" & Session("FileLoadID") & "' AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
		strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE FileSeqNum = '" & Session("FileLoadID") & "' AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
		
		'strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
		'strSQL = "SELECT " & strTOP & " * FROM tblCAPSNABCS WITH(NOLOCK) WHERE FileSeqNum = '" & Session("FileLoadID") & "' AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
		'strSQL = "SELECT " & strTOP & " * FROM qryCAPSCSFromDinersAuditLog WITH(NOLOCK) WHERE FileSeqNum = " & Session("FileLoadID") & " AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR Surname Like '%" & strSearch & "%')" & strWhere & strSort
...
'		If NOT IsNumeric(Session("PageCombo")) Then Session("PageCombo") = lngTotalRecords
		...
		Response.Write "<div class=""row""><div class=""col-12"">" & _
			...
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=CSFromDinersID&SortType=" & strOrderType & """> CS ID <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=EIDNo&SortType=" & strOrderType & """> EID <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=Surname&SortType=" & strOrderType & """> Name <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=CardStatus&SortType=" & strOrderType & """> Card Status <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col"">Card No.</th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=Status&SortType=" & strOrderType & """> Status  <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=FileDateTime&SortType=" & strOrderType & """> File Date <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col"" title=""Card Type""> File Load ID </th>" & _
			"<th scope=""col""><a href=""CSTransactionsNAB.asp?Sort=Notes&SortType=" & strOrderType & """> Change Details  <i class=""fa fa-sort""></i></a></th>" & _
...
'				strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=UnRemove&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-list""></i> Re-List</button>"
'			Case "Added to CS"
'
...
'				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=Cancel&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-circle""></i> Cancel</button>"
'			
'			Case Else
'				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=Cancel&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-circle""></i> Cancel</button>"
...
'				strAction = strAction & "<button type=""button"" class=""btn btn-outline-info btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=Remove&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-cross""></i> Remove</button>"
'
'				'strAction = strAction & "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=Email&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-mail""></i> Email</button>"
...
'			End Select
			...
			If trim(objRS("Status")) = "Awaiting Export" Then
				...
				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=CancelCS&CSFromDinersID=" & objrs("CSFromDinersID") & "&CSEID=" & objRS("EIDNo") & "&Status=Deleted'""; title=""Click to Remove from CS File""><i class=""fa fa-times""></i> Remove</button>"
				
			ElseIf objRS("Status") = "Deleted" Then
				'strStatusDisplay = "<span class=""badge badge-pill badge-danger"">" & strStatus & "</span>"
				strAction = "<button type=""button"" class=""btn btn-success btn-xs"" onclick=""self.location='CSTransactionsNAB.asp?Action=CancelCS&CSFromDinersID=" & objrs("CSFromDinersID") & "&CSEID=" & objRS("EIDNo") & "&Status=Awaiting Export'""; title=""Click to Add to CS File""><i class=""fa fa-plus""></i> Add</button>"
			...
			If IsNull(objRS("Surname")) Then
				strFullName = ""
			Else
				strFullName = trim(objRS("FirstName")) & " " & trim(objRS("Surname"))
			...
			Response.write "<TR><TD ><a data-toggle=""modal"" data-target=""#CSFromDinersMod"" HREF=""#"" onClick=""loadDoc(" & objRS("CSFromDinersID") & ")"">" & objRS(0) & "</a></TD><TD>" & objRS("EIDNo") & "</a></TD>" & _
					"<TD style=""font-size:12px;"" Title=""" & trim(objRS("FirstName")) & " " & trim(objRS("Surname")) & """><a Target=""_self"" HREF=""../CC/CardDetail.asp?CardNo=" & objRS("CardNumber") & """>" & strFullName & "</a></TD><TD><a Target=""_self"" HREF=""../CC/CardDetail.asp?CardNo=" & objRS("CardNumber") & """>" & objRS("CardStatus") & "</a></TD>" & _
...
'						strPages2 = strPages2 & "<li class=""page-item""><a class=""page-link"" href=""CSTransactionsNAB.asp?StartingRecord=" & lngTotalRecords - clng(Session("PageCombo")) & """ aria-label=""More""><span aria-hidden=""true"">&hellip;</span><span class=""sr-only"">More</span></a></li>"
'					End If
...
'						strPages2 = strPages2 & "<li class=""page-item""><a class=""page-link"" href=""CSTransactionsNAB.asp?StartingRecord=" & lngTotalRecords - (clng(Session("PageCombo"))*20) & """ aria-label=""More""><span aria-hidden=""true"">&hellip;</span><span class=""sr-only"">More</span></a></li>"
'					End If
'				Else
...
'					End If
					...
					'strPages = strPages & "<a href=""CSTransactionsNAB.asp?StartingRecord=" & (x * clng(Session("PageCombo"))) & """> " & x + 1 & " </a>"
'					strPages2 = strPages2 & "<li class=""page-item " & strActive & """><a class=""page-link"" href=""CSTransactionsNAB.asp?StartingRecord=" & (x * clng(Session("PageCombo"))) & """>" & x & "</a></li>"
					
					'strPages = strPages & "<a href=""CSTransactionsNAB.asp?StartingRecord=" & (x * 50) & """> " & x & " </a>"
					'strPages2 = strPages2 & "<li class=""page-item " & strActive & " -" & x & "|" & lngPage & """><a class=""page-link"" href=""CSTransactionsNAB.asp?Previous&StartingRecord=" & lngStartingRecord -50 & """>" & x & "</a></li>"
'					End If
'				End If
'			Next
...
End If
	...
	Response.Write "<div class=""btn-group btn-selector"" role=""group"" aria-label=""Basic example"">" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(1) & """ onClick=""self.location.href='CSTransactionsNAB.asp?ViewButton=All';""><i class=""fa fa-folder""></i> View All</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(3) & """ onClick=""self.location.href='CSTransactionsNAB.asp?ViewButton=NoChange';""><i class=""fa fa-thumbs-down""></i> View No Change</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(4) & """ onClick=""self.location.href='CSTransactionsNAB.asp?ViewButton=Change';""><i class=""fa fa-thumbs-up""></i> View Changes</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(5) & """ onClick=""self.location.href='CSTransactionsNAB.asp?ViewButton=Cancelled';""><i class=""fa fa-times""></i> View Cancels</button>" & _
				"</div>"
...
Dim objTextStream
	...
	objCon.Execute "UPDATE tblCAPSCSToNAB SEt Address1 = REPLACE(Address1,char(9),' ') WHERE charindex(char(9),Address1)>0"
	objCon.Execute "UPDATE tblCAPSCSToNAB SEt Address2 = REPLACE(Address2,char(9),' ') WHERE charindex(char(9),Address2)>0"
	objCon.Execute "UPDATE tblCAPSCSToNAB SEt Address3 = REPLACE(Address3,char(9),' ') WHERE charindex(char(9),Address3)>0"
	objCon.Execute "UPDATE tblCAPSCSToNAB SEt CardUpdateInd = 'E' WHERE CardUpdateInd Is Null"



	'Open a recordset of all of the NA File records yet to be exported
	strSQL = "SELECT * FROM tblCAPSCSToNAB WITH(NOLOCK) WHERE (FileSeqNum Is Null OR FileSeqNum = '') AND ([Status] = 'Awaiting Export' Or [Status] = '')"
	objRS.Open strSQL,objCon
	'objRS.Open "SELECT * FROM tblCAPSCSToNAB WITH(NOLOCK) WHERE CardType = '" & strCardType & "' AND (FileSeqNum Is Null OR FileSeqNum = '') AND ([Status] = 'Awaiting Export' Or [Status] = '')",objCon
	
	'Response.Write "SELECT * FROM tblCAPSCSToNAB WITH(NOLOCK) WHERE CardType = '" & strCardType & "' AND (FileSeqNum Is Null OR FileSeqNum = '') AND ([Status] = 'Awaiting Export' Or [Status] = '')"
	...
	'lngFileLoadID = 0
			...
			Do Until objRS.EOF
				...
				strError = strError & CheckForNull(objRS("FirstName"),"Given Names",intRecordCount)
				...
				If IsNull(objRS("CSFromDinersID")) = False Then
				...
				'UPDATE -- Changed phone number to pad space left instead of pad digits THEN replaced phone numbers with variables above for nulls/empties to have zeroes
					objTextStream.WriteLine "D " & PadSpaceLeft(objRS("EIDNo"),10) & PadSpaceRight(objRS("CardNo"),19) & PadSpaceLeft(objRS("CardUpdateInd"),2) & PadSpaceLeft(objRS("CardExpiryDate"),8) & PadSpaceLeft(objRS("CardStatus"),2) & PadSpaceLeft(objRS("Title"),12) & PadSpaceLeft(objRS("Surname"),25) & PadSpaceLeft(objRS("FirstName"),30) & PadSpaceLeft(objRS("NameOnCard"),26) & PadSpaceLeft(objRS("Address1"),40) & PadSpaceLeft(objRS("Address2"),40) & PadSpaceLeft(objRS("Address3"),40) & PadSpaceLeft(objRS("Suburb"),25) &  PadSpaceLeft(objRS("State"),4) & PadSpaceLeft(objRS("Postcode"),12) & PadSpaceRight(strHomePhone,12) &  PadSpaceRight(strWorkPhone,12) & PadSpaceRight(strMobilePhone,12) & PadSpaceLeft(objRS("Email"),70) & PadSpaceLeft(strReportGroup,8) & PadSpaceLeft(objRS("CreditLimit"),11) & PadSpaceLeft("",78)  
				...
				'intCount = intCount + 1
					...
					'Call ExportCSRecord (objRS("CSFromDinersID"),lngFileLoadID,intRecordCount)
					'Call ExportCSRecord (objRS("CSFromDinersID"),strNextFileNumber,intCount)'intRecordCount)
					Call ExportCSRecord (objRS("CSFromDinersID"),strNextFileNumber,intRecordCount)
				'End If
			objRS.Movenext
			Loop
...
Public Sub ExportCSRecord(lngCSFromDinersID,lngBatchNumber,x)
...
Dim intRecord
  	...
  	With objCmd
		...
		If x = 1 Then
			.Parameters.Append objCmd.CreateParameter("CSFromDinersID", adVarChar, adParamInput,10)
...
Public Sub RemoveCSRecord(lngCSFromDinersID, strEmployeeID, strStatus)
...
Dim intRecord
  	...
  	With objCmd
		...
		End With
	   'response.write "spCAPSCSFileRemoveCard " & lngCSFromDinersID & "," & Session("UserID") & "," & strStatus
		objCmd.Execute        
		...
		If intRecord = 0 Then
			If strStatus = "Deleted" Then
				Response.Write "<div class=""alert alert-danger"" role=""alert"">CS Record for " & strEmployeeID & " NOT Removed from CS File! An Error has occurred. See System Admin with CS File ID: " & lngCSFromDinersID & " </div>"
			Else
				Response.Write "<div class=""alert alert-danger"" role=""alert"">CS Record for " & strEmployeeID & " NOT Added to the CS File! An Error has occurred. See System Admin with CS File ID: " & lngCSFromDinersID & " </div>"
			End If
		Else
			If strStatus = "Deleted" Then
...
'Description:	Loads all Batch Numbers to a list for selecting and searching/filtering
	...
	objRS.Open "SELECT * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE [FileType] = 'CSFromNAB' AND [Deleted] = 'N' ORDER By [FileSeqNum] DESC",objCon	
  
	Response.write "<OPTION value=""0"">Select a Batch to View...</OPTION><OPTION value=""0"">NAB CS Transactions </OPTION>"
...
Public Function Get_Batch_Date(strFileLoadID)
	...
	objRS2.Open "SELECT * FROM tblCAPSFileLoad WHERE FileType = 'CSFromNAB' AND FileSeqNum = '" & strFileLoadID & "'",objCon
