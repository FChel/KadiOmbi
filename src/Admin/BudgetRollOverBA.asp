...
var versionID;
    ...
    if (isWhitespace(frm.BusinessAreaID.value) || frm.BusinessAreaID.value == "0") {
        varAlert += "Business Area Being Rolled over Cannot Be Blank. \n \n";
...
<form action="BudgetRolloverBA.asp?Action=Save" method="POST" id="frm" name="frm">

<table class="custom_table" WIDTH="100%" CELLSPACING="1" CELLPADDING="1" >
	<tr>
		<th class="custom_table_th_4" style="width:20%" Height="25px">&nbsp;Business Area Budget Rollover Screen</th>
	...
	<tr><td colspan="2">&nbsp</td></tr>
	 ...
	 <tr>
		<th class="custom_table_th_2" height="20px">Business Area</th>			
		<td Width="40%" class="custom_table_td_1"><select Style="Width:15%" tabindex="3" id="BusinessAreaID" name="BusinessAreaID"><option value="0">--</option>			
		
		<%
		 'Open a recordset and build list using recordset details.			
		objRS.Open "SELECT * FROM tblBusinessArea WHERE BudgetID = " & Session("BudgetID") & " AND BusinessAreaID <> 1000 AND Active = 'Y'",objCon
		
		Do until objRS.eof	
		
			If lngBusinessAreaID = clng(objRS("BusinessAreaID")) Then
...
Sub SaveRecord()
		...
		objCmd.Execute	
           ...
           End With 
            ...
            strMessage = "<B>BUSINESS AREA BUDGET HAS BEEN ROLLED OVER.</B>"
