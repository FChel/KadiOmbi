...
function SelectFileSeqNum(varFileSeqNum) {
	...
	{
	self.location = "UploadANZDetail.asp?Action=Search&FileSeqNum=" + varFileSeqNum + "&EIDNo=" + document.getElementById('EmpIDAnchor').text;
	}
}
function SelectEmp(varEmpID) {
	...
	{
	self.location = "UploadANZDetail.asp?Action=Search&EIDNo=" + varEmpID + "&FileSeqNum=" + document.getElementById('FileAnchor').text;
	}
}
</script>
...
<form action="UploadANZDetail.asp?Action=Save&chkDelete="  method="POST" enctype="multipart/form-data" id="frm" name="frm">
	<!--scrolling File Seq Num Modal -->
            <div class="modal fade" id="exampleModalScrollable" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
              <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalScrollableTitle">ANZ Cardlist File No.</h5>
...
<!--scrolling Employee Modal -->
            <div class="modal fade" id="EmployeeModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
              <div class="modal-dialog modal-dialog-scrollable" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalScrollableTitle">ANZ Cardlist Employee ID</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <i class="bx bx-x"></i>
                    </button>
                  </div>
...
<div class="content-wrapper">
 ...
 <button type="button" class="btn btn-secondary btn-xs mr-1 mb-1" onclick="self.location='UploadANZ.asp'"><i class="fa fa-arrow-left"></i> Back To ANZ Summary</button>
...
Else
	strWhere = " AND EmployeeID = '" & strEID & "'"
End If
...
End If
		...
		objRS.Open "SELECT " & strTop & " * FROM qryCAPSANZCardlistReco WITH(NOLOCK) "  & strWhere,objCon
		
		    If objRS.eof Then
		        Response.Write"<table Class=""table"" WIDTH=""100%"" BORDER=""1"" CELLSPACING=""1"" CELLPADDING=""1"">" & _
                "<tr><th colspan=7 style=""text-align:left"">No ANZ CArdlist data for " & strWhere & "</B></th></tr>" & _
		    ...
		    Else
                ...
                "<tr><th colspan=""15"" style=""text-align:left"">ANZ Cardlist data loaded for " & strWhere & "</th>" & _
                ...
                "<th>Suburb</th><th>State</th><th style=""background-color:white; color:black;"">Cards</th></tr>"
		        
		    End If
		    
		    Do until objRS.eof
		        'If objRS("GLCode") <> 0 Then
			    Response.Write "<TR class='clickable-row' data-href='DisplayDataset.asp?tbl=tblCAPSANZCardlist&W=WHERE ANZCardlistID=" & objRS("ANZCardlistID") & "' data-target='_blank'><TD>" & objRS("ANZCardlistID") & "</TD>" & _
			                    "<TD style=""text-align:center"">" & objRS("DateLoaded") & "</TD><TD style=""text-align:center"">" & objRS("FileID") & "</TD><TD style=""text-align:center"">" & objRS("EmployeeID") & "</TD>" & _
			                    "<TD style=""text-align:center"">" & objRS("CardNumber") & "</TD><TD style=""text-align:center"">" & objRS("Expiry") & "</TD><TD style=""text-align:center"">" & objRS("Status") & "</TD>" & _
			                    ...
			                    "<TD style=""text-align:center; background-color:white; color:black;"">" & objRS("CardID") & "</TD></TR>"
    			'End If
