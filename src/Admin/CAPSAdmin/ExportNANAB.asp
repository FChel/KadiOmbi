...
<%
'Description:	New Card file export for NAB
...
If Not IsEmpty(Request.QueryString("CardType")) Then
	...
	strCardType = "NAB" 'Request.QueryString("CardType")
...
End If
	...
	Else
			...
			strNATransactionPage = "NATransactionsNAB.asp"
	...
	'Get the card type button to toggle between DPC and DTC
		...
		'strCardTypeButton = "<button type=""button"" class=""btn btn-outline-secondary"" title=""DPC Selected.  Click to Change to DTC"" onClick='window.location=""ExportNANAB.asp?CardType=DTC""'><i class=""fa fa-dollar-sign""></i>NAB</button>"
		strCardTypeButton = ""
...
function loadNA(CardType) {
  ...
  xhttp.open("GET", "../../CC/AJAX/GetNAFileNAB.asp", true);
  xhttp.send();
}
...
function DatePickChange(CardType) {
	...
	self.location="ExportNANAB.asp?CardType=" + CardType + "&FileDate=" + document.getElementById("CSDate").value;
}
         ...
         <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="compareModalLabel">
                  NA File to NAB - <%=strCardType & " : Previous File Number: " & strNextNAFileNumber%>
                </h5>
  ...
  <div class="col-4">
    <div class="card">
     <div class="card-header">
          <h4 class="card-title"><img src="../../CC/img/Diners2.png" height="40px" width="50px" title="Diners"> NA To NAB File Export <%=strCardTypeButton%></h4>
        </div>
      <div class="card-content">
        <div class="card-body">
			<p class="card-text">
				New NAB Applications which have been processed in CAPS but not yet exported to NAB (in the NA file) will appear below.  
...
<fieldset class="form-group">
	...
	<button type="button" class="btn btn-outline-secondary btn-sm" onClick="self.location='NATransactionsNAB.asp?CardType=<%=strCardType%>'"><i class="fa fa-file"></i> View Details </button>
...
If Not IsEmpty(Request.QueryString("BatchNo")) Then
	If IsNull(Request.QueryString("BatchNo")) or Request.QueryString("BatchNo")= "" Then 
		strWhere = "WHERE Status = 'Added To NA' AND Left(CardTypeSub,3)  = '" & strCardType & "' AND [BatchNumber] = 0"
	Else
		strWhere = "WHERE Status = 'Added To NA' AND Left(CardTypeSub,3)  = '" & strCardType & "' AND FileSeqNum = " & Request.QueryString("BatchNo") & ""
	
		If Request.QueryString("BatchNo") = 0 Then strWhere = strWhere & " OR BatchNumber IS NULL"
	End If
Else
	strWhere = "WHERE Status = 'Added To NA' AND Left(CardTypeSub,3) = '" & strCardType & "' AND [BatchNumber] = 0 AND [Status] <> 'Deleted'"
End If



objRS.Open "SELECT TOP 50 * FROM qryCAPSNAToNAB "  & strWhere,objCon
		    ...
		    Else
                ...
                "<tr><th colspan=""16"" style=""text-align:left"">Sample of Existing NA To NAB Data ready for export</th></tr>" & _
		        "<tr><th Style=""width:20px;"">NA To NAB ID</th>" & _
				"<th>EmployeeID</th>" & _
				"<th>Record Type</th><th>Surname</th>" & _	
				"<th>Card Type</th>" & _
		        "<th>Status</th>" & _
		    ...
		    Do until objRS.eof
			    ...
			    Response.Write "<TR class='clickable-row' data-href='../DisplayDataset.asp?tbl=qryCAPSNAToDiners&W=WHERE NABNAID=" & objRS("NABNAID") & "' style=""cursor: pointer;""><TD>" & objRS("NABNAID") & "</TD>" & _
			                    "<TD style=""text-align:center"">" & objRS("EIDNo") & "</TD><TD style=""text-align:center"">" & objRS("RecordType") & "</TD><TD style=""text-align:center"">" & objRS("Surname") & "</TD>" & _
			                    "<TD style=""text-align:center"">" & objRS("CardTypeSub") & "</TD><TD style=""text-align:center"">" & objRS("Status") & "</TD><TD style=""text-align:center"">" & objRS("BatchNumber") & "</TD><TD style=""text-align:center"">" & objRS("DateUpdated") & "</TD>" & _
...
'Response.Write "Card Type2 = " & strCardType & "<BR>"
	...
	dteBatchDateFormat = Year(dteBatchDate) & "-" & Right("0" & Month(dteBatchDate), 2) & "-" & Right("0" & Day(dteBatchDate), 2)
		...
		objRS.Open "SELECT TOP 6 * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE Deleted = 'N' AND DateLoaded > '" & dteBatchDate & "' AND FileType = 'NAFileNAB' ORDER BY FileSeqNum DESC",objCon	
		'Response.Write "SELECT TOP 6 * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE Deleted = 'N' AND DateLoaded > '" & dteBatchDate & "' AND FileType = 'NAFileNAB' ORDER BY FileSeqNum DESC"
		    If objRS.EOF Then
		        Response.Write"<table Class=""table"" WIDTH=""100%"" BORDER=""1"" CELLSPACING=""1"" CELLPADDING=""1"">" & _
		    ...
		    Do until objRS.eof
				...
				If strStatus = "Imported" Then
					strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ModApp"" onclick=""self.location='../NATransactionsNAB.asp?CardType=" & strCardType & "&Action=Process&FileLoadID=" & objRS("FileSeqNum") & "'""; title=""Click to Process the NA File and update changes to cards in CAPS from the CS File loaded " & objRS("DateLoaded") & """>Process</button>"
				...
				strView = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ModApp"" onclick=""self.location='NATransactionsNAB.asp?CardType=" & strCardType & "&FileLoadID=" & objRS("FileSeqNum") & "&BatchNumber=" & objRS("FileSeqNum") & "&ViewButton=All'""; title=""Click to view details of the NA File exported " & objRS("DateLoaded") & """>View</button>"
								
				Response.Write "<TR><TD><a href=""NATransactionsNAB.asp?CardType=" & strCardType & "&BatchNo=" & objRS("FileSeqNum") & """>" & objRS("FileSeqNum") & "</A></B></TD>" & _
							"<TD style=""text-align:center"">" & objRS("CardCount") & "</TD>" & _
							"<TD style=""text-align:center"">" & objRS("DTCCount") & "</TD><TD style=""text-align:center"">" & objRS("DPCCount") & "</TD><TD style=""text-align:center"">" & objRS("CMCCount") & "</TD>" & _
...
'Get the System Parameter for the fileName
	...
	strFileNameDefault = GetSystemAdmin("NAFileStartNAB")
...
Set objFSO = CreateObject("Scripting.FileSystemObject")
	...
	Response.Write"<table Class=""table table-bordered mb-0 newd"" cellspacing=""0"" cellpadding=""0"">" & _
			"<tr><th style=""text-align:left"" Title=""Final Export Location for NA File: " & strServer & """>Files Exported <i class=""fa help-tooltip fa-question-circle"" data-toggle=""tooltip"" title=""Files on the CAPS Server waiting to be sent to NAB.""></i></th></tr>"
