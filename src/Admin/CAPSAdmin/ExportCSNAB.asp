...
End If
	...
	Else
		strNextCSFileNumber = GetSystemAdmin("CMFileNumberTo")
		If IsNumeric(strNextCSFileNumber) Then strNextCSFileNumber = PadDigits(strNextCSFileNumber,6)
...
{   
    ...
    {   
        ...
        {
            if(window.confirm('This will overwrite any existing CS From NAB file data! \n \n Continue?')==true)
                {
...
function DatePickChange() {
	self.location="ExportCSNAB.asp?FileDate=" + document.getElementById("CSDate").value;
}
...
function loadCS() {
  ...
  xhttp.open("GET", "../../CC/AJAX/GetCSFileNAB.asp?Deleted=N", true);
  xhttp.send();
...
function loadCSDeleted() {
  ...
  xhttp.open("GET", "../../CC/AJAX/GetCSFileNAB.asp?Deleted=Y", true);
  xhttp.send();
     ...
     <div class="modal-dialog modal-large modal-dialog-right modal-dialog-scrollable">
         <div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="window.open('CSToDinersExportExcel.asp')"><i class="fa fa-file"></i> Export CM File to Excel</button>
				<button type="button" class="btn btn-primary float-right" data-toggle="modal" data-target="#ModalApprove" data-CSNumber="<%=strNextCSFileNumber%>" onClick="ConfirmExport(this);" Title="Click to send the Next CS File"><i class="fa fa-file"></i> Export CM File</button>
...
<div class="modal fade" id="ModalApprove" tabindex="-1" role="dialog" aria-labelledby="ModalApprove" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalApproveTitle" style="font-weight:bold;">CM File Export Confirmation</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <span>Export CM File Number: <input type="text" name="CSFileExport" id="CSFileExport" style="border:0; font-weight:bold; width:100px; text-align:right;" value="<%=strNextCSFileNumber%>" >?</span><br><br>
      </div>
      <div class="modal-footer">
		<button type="button" class="btn btn-primary" onClick='window.location="../CSTransactionsToNAB.asp?Action=ExportCS"'><i class="fa fa-check"></i> Yes</button>
...
<form action="ExportCSNAB.asp?Action=Save&chkDelete="+frm.chkDelete.value  method="POST" enctype="multipart/form-data" id="frm" name="frm">
  ...
  <div class="col-3">
    <div class="card">
     <div class="card-header">
          <h4 class="card-title"><img src="../../CC/img/Diners2.png" height="40px" width="50px" title="Diners"> CM To NAB File Export</h4>
        </div>
      <div class="card-content">
        <div class="card-body">
			<p class="card-text">
				Changes to NAB cards which have been processed in CAPS but not yet exported to NAB (in the CM file) will appear below.  
				Click Export to create the export file and process all records below.
			</p>
...
<fieldset class="form-group">
    <button type="button" class="btn btn-outline-secondary btn-sm" data-toggle="modal" data-target="#CSModal" HREF="#" onClick="loadCS();"><i class="fa fa-file"></i> Export </button>
	<button type="button" class="btn btn-outline-secondary btn-sm" onClick="self.location='../CSTransactionsToNAB.asp?FileLoadID=0'"><i class="fa fa-file"></i> View Details </button>
</fieldset>
<!--</div>-->
</p>
 <script>
...
If Not IsEmpty(Request.QueryString("BatchNo")) Then
	...
	End If
Else
	strWhere = "WHERE FileSeqNum Is NULL OR FileSeqNum = ''"
End If

'First Get the Total records Ready to be exported for display
objRS.Open "SELECT Count(*) AS [CountCS] FROM qryCAPSCSToNAB "  & strWhere,objCon
...
'Get the Top so as not to confuse users
intTopCount = 100

If cint(intTopCount) > cint(lngRecords) Then
	intTopCount=lngRecords
Else
	intTopCount = 100
End If

Response.Write "SELECT TOP 100 * FROM qryCAPSNABCM "  & strWhere

objRS.Open "SELECT TOP 100 * FROM qryCAPSNABCMDisplayDetails "  & strWhere,objCon
		
		    If objRS.eof Then
		        Response.Write"<table Class=""table"" WIDTH=""100%"" BORDER=""1"" CELLSPACING=""1"" CELLPADDING=""1"">" & _
                "<tr><th colspan=7 style=""text-align:left"">There is no CM To NAB data ready for export</B></th></tr>" & _
		    ...
		    Else
                ...
                "<tr><th colspan=""16"" style=""text-align:left"">Sample of Existing CM To NAB Data ready for export. Top " & intTopCount & " of <b>" & lngRecords & "</b> Total to be exported.</th></tr>" & _
		        "<tr><th Style=""width:20px;"">CM To NAB ID</th>" & _
				"<th>Card Type</th><th>Card Type Sub</th>" & _
				"<th>EmployeeID</th>" & _
				"<th>Card No</th><th>Status</th>" & _	
		        "<th>Title</th>" & _
	 	        "<th>FirstName</th>" & _	
	 	        "<th>Surname</th>" & _
		    ...
		    Do until objRS.eof
			    ...
			    Response.Write "<TR class='clickable-row' data-href='../CSTransactionsToNAB.asp?BatchNo=" & objRS("FileSeqNum") & "&EIDNo=" & objRS("EIDNo") & "' style=""cursor: pointer; ; font-size:12px;""><TD>" & objRS("CSToDinersID") & "</TD>" & _
								"<TD style=""text-align:center; font-size:12px;"">" & objRS("CardType") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("CardTypeSub") & "</TD>" & _
			                    "<TD style=""text-align:center; font-size:12px;"">" & objRS("EIDNo") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("CardNumber") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("CardStatus") & "</TD>" & _
			                    "<TD style=""text-align:center; font-size:12px;"">" & objRS("Title") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("FirstName") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("CDMCSurname") & "</TD>" & _
			                    "<TD style=""text-align:cente; font-size:12px;r"">" & objRS("Address1") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("Address2") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("Address3") & "</TD>" & _
			                    "<TD style=""text-align:center; font-size:12px;"">" & objRS("Suburb") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("StateCode") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("PostCode") & "</TD>" & _
								"<TD style=""text-align:center; font-size:12px;"">" & objRS("EmailAddress") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("Status") & "</TD><TD style=""text-align:center; font-size:12px;"">" & objRS("FileSeqNum") & "</TD>" & _
			                    "</TR>"
    			'End If
...
Dim strDateUpdatedTitle
	...
	dteBatchDateFormat = Year(dteBatchDate) & "-" & Right("0" & Month(dteBatchDate), 2) & "-" & Right("0" & Day(dteBatchDate), 2)
		...
		objRS.Open "SELECT TOP 10 * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE Deleted = 'N' AND DateLoaded < '" & dteBatchDate & "' AND FileType = 'CMToNAB' ORDER BY FileSeqNum DESC",objCon
		
		'objRS.Open "SELECT TOP 50 * FROM qryCAPSCSFromDinersSummary WHERE DateUpdated > '" & dteBatchDate & "' ORDER BY BatchNo DESC",objCon
		
		    If objRS.EOF Then
		        Response.Write"<table Class=""table"" WIDTH=""100%"" BORDER=""1"" CELLSPACING=""1"" CELLPADDING=""1"">" & _
                "<tr><th colspan=""10"" style=""text-align:left"">There is no CM To NAB data loaded (Before <input class=""DateClick2"" type=""date"" style=""color:red;"" value=""" & dteBatchDateFormat & """ id=""CSDate"" onChange=""DatePickChange();""/>)</th></tr>" 
		        
		    Else
		    
		         Response.Write"<table Class=""table table-bordered table-compact mb-0 newd"" cellspacing=""0"" cellpadding=""0"">" & _
                "<tr><th colspan=""8"" style=""text-align:left"">CM To NAB Summary (Before <input class=""DateClick2"" type=""date"" style=""color:red;"" value=""" & dteBatchDateFormat & """ id=""CSDate"" onChange=""DatePickChange();"" />)</th></tr>" & _
		    ...
		    Do until objRS.eof
				...
				Response.Write "<TR><TD><a href=""ExportCSNAB.asp?BatchNo=" & objRS("FileSeqNum") & """>" & objRS("FileSeqNum") & "</A></B></TD>" & _
							...
							"<TD style=""text-align:center"">" & objRS("DTCCount") & "</TD><TD style=""text-align:center"">" & objRS("DPCCount") & "</TD><TD style=""text-align:center"">" & objRS("CMCCount") & "</TD><TD style=""text-align:center"">" & objRS("Status") & "</TD>" & _
...
Set objFSO = CreateObject("Scripting.FileSystemObject")
	...
	Response.Write"<table Class=""table table-bordered table-compact mb-0 newd"" cellspacing=""0"" cellpadding=""0"">" & _
			"<tr><th style=""text-align:left"">Files Exported <i class=""fa help-tooltip fa-question-circle"" data-toggle=""tooltip"" title=""Files on the CAPS Server waiting to be sent to NAB.""></i></th></tr>"
