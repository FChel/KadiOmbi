...
var check = false
  ...
  if ( check==true) {
	...
	//LoadCDMCAJAX('on');
   	self.location="UploadCDMC.asp?Action=SaveFileLocal&Delete=on"
    } else {
        // Returns false if not checked
	//LoadCDMCAJAX('off');
	
	self.location="UploadCDMC.asp?Action=SaveFileLocal&Delete=off"
    }
...
{
	document.getElementById('Progress').style.display = "inline";
	self.location="UploadCDMC.asp?Action=SaveFileLocal&Delete=off&ActionType=Service"
}

function synchCDMC()
{
	document.getElementById('Progress3').style.display = "inline";
	self.location="UploadCDMC.asp?Action=SynchCDMC"
}
...
document.getElementById('Progress4').style.display = "inline";
	...
	//self.location="UploadCDMC.asp?Action=Save12"
	//self.location="UploadCDMC.asp?Action=SaveFileLocal&Delete=off&ActionType=Service"
	self.location="UploadCDMC.asp?Action=SaveFileLocal&Delete=off&ActionType=" + varActionType
...
document.getElementById('Progress3').style.display = "inline";
	...
	self.location="UploadCDMC.asp?Action=ProcessCDMC"
...
function RunProcessA()
{
   ...
   self.location="UploadCDMC.asp?Action=ProcessA&FileSeqNum=" + FileSeqNum + "&FileName=" + FileName + "&FileLoadID=" + FileLoadID 
...
function RunProcessB()
{
   ...
   self.location="UploadCDMC.asp?Action=ProcessB&FileSeqNum=" + FileSeqNum + "&FileName=" + FileName + "&FileLoadID=" + FileLoadID 
...
<main class="main py-3">
      <div class="container">
<form action="UploadCDMC.asp?Action=Save" method="POST" enctype="multipart/form-data" id="frm" name="frm">
  ...
  <div class="col-3">
    <div class="card">
     <div class="card-header">
          <h4 class="card-title"><img src="../CC/img/DFG_Logo.png" height="40px" width="100px" title="Certificate and Directory Management Centre"> CDMC Load </h4>
		...
		'objRS.Open "SELECT TOP 50 * FROM qryCAPSCSFromDinersSummary WHERE DateUpdated > '" & dteBatchDate & "' ORDER BY BatchNo DESC",objCon
		    ...
		    Do until objRS.eof
				...
				If strStatus = "Imported" AND objRS("Deleted") = "N" Then
					...
					strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ProcessARun"" onclick=""LoadProcessA('" & objRS("FileSeqNum") & "','" & objRS("FileName") & "','" & objRS("FileLoadID") & "');"">Process History</button>"
				...
				ElseIf strStatus = "History Processed" AND objRS("Deleted") = "N" Then
					'strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ModApp"" onclick=""self.location='UploadCDMC.asp?Action=ProcessB&FileSeqNum=" & objRS("FileSeqNum") & "&FileName=" & objRS("FileName") & "&FileLoadID=" & objRS("FileLoadID") & "'""; title=""Click to Process the CDMC File and update changes to Corporate Directory " & objRS("DateLoaded") & """>Process Details</button>"
					strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ProcessBRun"" onclick=""LoadProcessB('" & objRS("FileSeqNum") & "','" & objRS("FileName") & "','" & objRS("FileLoadID") & "');"">Process Details</button>"
				Else
...
'---Start Excel driver upload
			...
			'Else
				...
				'If lngFileID = 1 or strDeleteCheck = "on" Then
					...
					'Response.Write "TRUNCATE TABLE tblCAPSCDMC"
				'Else
					'Response.write "<div class=""content-body""><section id=""basic-alerts""><div class=""alert alert-danger alert-dismissible mb-2"" role=""alert""><button type=""button"" class=""close"" data-dismiss=""alert"" aria-label=""Close"">" & _
					'	"<span aria-hidden=""true"">&times;</span></button>" & _
					'	"<div class=""d-flex align-items-center""><i class=""bx bx-error""></i>" & _
