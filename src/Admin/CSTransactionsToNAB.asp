...
Dim strHeaderText
	...
	Else 
		If IsNull(Session("FileLoadID")) OR Session("FileLoadID") = "" Then Session("FileLoadID")= 0 End If
	End If
	
	
	If Not IsEmpty(Request.QueryString("BatchNo")) Then
		Session("BatchNo") = Request.QueryString("BatchNo")
	End If
	...
	If Not IsEmpty(Request.QueryString("BatchNo")) Then
		Session("FileLoadID") = Request.QueryString("BatchNo")
	...
	IF Session("FileLoadID") = "0" OR Session("FileLoadID") = "" Then
		strHeaderText = "<span style=""font-weight:normal; font-style:italic; font-size:16px;"">CM File to Be Exported Today</span>"
	Else
...
function ChangeBatch() {
	...
	//alert('CSTransactionsToNAB.asp?CardType=' + CardType + '&FileLoadID='+result);
	self.location='CSTransactionsToNAB.asp?FileLoadID='+result
	//self.location='CSTransactionsToNAB.asp?CardType=' + CardType + '&FileLoadID='+result
...
function ChangePage() {
	...
	self.location = 'CSTransactionsToNAB.asp?PageCombo=' + result;
	//alert(result);
	//document.getElementById('CardType').value=result;
...
function loadDoc(varID) {
  ...
  xhttp.open("GET", "../CC/AJAX/GetCSToDinersNAB.asp?CSToDinersID=" + varID + "", true);
  xhttp.send();
}
         ...
         <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="compareModalLabel">
                  CM To NAB File Detail - <%=strCardType%>
                </h5>
...
<div class="modal fade" id="ModalSelectBatch" tabindex="-1" role="dialog" aria-labelledby="ModalSelectBatch" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalApproveTitle" style="font-weight:bold;">CM File To/Export Batch Number</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div id='tbl-container'>
  <form action="CSTransactionsToNAB.asp?Action=Search" method="POST" id="frm" name="frm">
	...
	<section class="breadcrumbs py-2">
		<div class="row">
			<div class="col-md-10">
				...
				<h4 class="text-left" data-toggle="modal" data-target="#ModalSelectBatch"  Title="Click to Select a CS File to View">CS File To NAB - <%="File Load ID : " & strHeaderText%>&nbsp;&nbsp;- Date Loaded : <%=Get_Batch_Date(Session("FileLoadID"))%> <button type="button" class="btn btn-primary" onClick="#" data-toggle="modal" data-target="#ModalSelectBatch"  Title="Click to Select a CS File to View"><i class="fa fa-file"></i> Select File</button></h4>
			</div>
			<div class="col-md-2">
...
If strSearch = "" OR ISNull(strSearch) Then
	...
	IF Session("FileLoadID") = "0" Then
		If Request.QueryString("AuditFor") <> "" THEN
			'strSQL = "SELECT " & strTOP & " * FROM qryCAPSNABCMDisplayDetails WITH(NOLOCK) WHERE (FileSeqNum = '" & Session("FileLoadID") & "') AND [CSToDinersID] > 0 " & strWhere & strSort
			strSQL = "SELECT * FROM qryCAPSNABCMDisplayDetails WITH(NOLOCK) WHERE FileSeqNum = '" & Session("FileLoadID") & "' AND (EIDNo Like '%" & Request.QueryString("AuditFor") & "%' OR FirstName Like '%" & Request.QueryString("AuditFor") & "%' OR CDMCSurname Like '%" & Request.QueryString("AuditFor") & "%')" & strWhere & strSort
			'response.write "a"
		Else
			...
			strSQL = "SELECT " & strTOP & " * FROM qryCAPSNABCMDisplayDetails WITH(NOLOCK) WHERE (FileSeqNum = '' OR FileSeqNum IS NULL) AND [CSToDinersID] > 0 " & strWhere & strSort	
			'response.write "b"
...
Else
	If Session("FileLoadID") = 0 THEN
		...
		Else
			...
			strSQL = "SELECT " & strTOP & " * FROM qryCAPSNABCMDisplayDetails WITH(NOLOCK) WHERE (FileSeqNum = '' OR FileSeqNum Is NULL OR FileSeqNum =" & Session("FileLoadID") & ") AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR CDMCSurname Like '%" & strSearch & "%')" & strWhere & strSort
			'response.write "f"
	...
	Else
		...
		strSQL = "SELECT " & strTOP & " * FROM qryCAPSNABCMDisplayDetails WITH(NOLOCK) WHERE FileSeqNum = '" & Session("FileLoadID") & "' AND (EIDNo Like '%" & strSearch & "%' OR FirstName Like '%" & strSearch & "%' OR CDMCSurname Like '%" & strSearch & "%')" & strWhere & strSort
		'response.write "g"
...
'response.write strSQL
	...
	If objRS.EOF Then
		Response.Write "<TR><TH colspan=""10"" Style=""text-align:center;"">No CS To NAB records for " & strRecordMessage & "</TH>" & _
...
'		If NOT IsNumeric(Session("PageCombo")) Then Session("PageCombo") = lngTotalRecords
		...
		Response.Write "<div class=""panel panel-light mb-3""><div class=""panel-header""><h4></h4>" & _
                  "<span class=""panel-subheader"">Displaying " & lngTotalRecords & " CS To NAB records </span></div></div>"
		...
		Response.Write "<div class=""row""><div class=""col-12"">" & _
			...
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=CSToDinersID&SortType=" & strOrderType & """> CS ID <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=EIDNo&SortType=" & strOrderType & """> EID <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=Surname&SortType=" & strOrderType & """> Name <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=CardStatus&SortType=" & strOrderType & """> Card Status <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col"">Card No.</th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=Status&SortType=" & strOrderType & """> Status  <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=FileDateTime&SortType=" & strOrderType & """> File Date <i class=""fa fa-sort""></i></a></th>" & _
			"<th scope=""col"" title=""Card Type""> File Load ID </th>" & _
			"<th scope=""col""><a href=""CSTransactionsToNAB.asp?Sort=Notes&SortType=" & strOrderType & """> Change Details  <i class=""fa fa-sort""></i></a></th>" & _
	...
	If objRS.EOF Then
		Response.Write "<TR><TH colspan=""10"" Style=""text-align:center;"">No CS From NAB records " & strRecordMessage & "</TH>" & _
				"<TH colspan=""3"" style=""text-align:center;""></TH></TR>"
	End If
...
'				strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=UnRemove&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-list""></i> Re-List</button>"
'			Case "Added to CS"
'
...
'				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=Cancel&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-circle""></i> Cancel</button>"
'			
'			Case Else
'				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=Cancel&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-circle""></i> Cancel</button>"
...
'				strAction = strAction & "<button type=""button"" class=""btn btn-outline-info btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=Remove&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-cross""></i> Remove</button>"
'
'				'strAction = strAction & "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=Email&CardID=" & objrs("CardID") & "&CardEID=" & objrs("EmployeeID") & "'"";><i class=""fa fa-minus-mail""></i> Email</button>"
...
'			End Select
			...
			If trim(objRS("Status")) = "Awaiting Export" Then
				...
				strAction = "<button type=""button"" class=""btn btn-danger btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=CancelCS&CSToDinersID=" & objrs("CSToDinersID") & "&CSEID=" & objRS("EIDNo") & "&Status=Deleted'""; title=""Click to Remove from CS File""><i class=""fa fa-times""></i> Remove</button>"
				
			ElseIf objRS("Status") = "Deleted" Then
				'strStatusDisplay = "<span class=""badge badge-pill badge-danger"">" & strStatus & "</span>"
				strAction = "<button type=""button"" class=""btn btn-success btn-xs"" onclick=""self.location='CSTransactionsToNAB.asp?Action=CancelCS&CSToDinersID=" & objrs("CSToDinersID") & "&CSEID=" & objRS("EIDNo") & "&Status=Awaiting Export'""; title=""Click to Add to CS File""><i class=""fa fa-plus""></i> Add</button>"
			...
			If IsNull(objRS("CDMCSurname")) Then
				strFullName = ""
			Else
				strFullName = trim(objRS("FirstName")) & " " & trim(objRS("CDMCSurname"))
			...
			Response.write "<TR><TD ><a data-toggle=""modal"" data-target=""#CSFromDinersMod"" HREF=""#"" onClick=""loadDoc(" & objRS("CSToDinersID") & ")"">" & objRS("CSToDinersID") & "</a></TD><TD>" & objRS("EIDNo") & "</a></TD>" & _
					"<TD style=""font-size:12px;"" Title=""" & trim(objRS("FirstName")) & " " & trim(objRS("CDMCSurname")) & """><a Target=""_self"" HREF=""../CC/CardDetail.asp?CardNo=" & objRS("CardNumber") & """>" & strFullName & "</a></TD><TD><a Target=""_self"" HREF=""../CC/CardDetail.asp?CardNo=" & objRS("CardNumber") & """>" & objRS("CardStatus") & "</a></TD>" & _
					"<TD style=""font-size:12px;"">" & MaskCard(objRS("CardNumber")) & "</TD><TD >" & strStatus & "</TD>" & _
					"<TD style=""font-size:12px; background-color:#e6eeff;"">" & objRS("FileDateTime") & "</TD><TD style=""font-size:12px; background-color:#e6eeff;"">" & objRS("FileSeqNum") & "</TD><TD style=""font-size:12px; background-color:#e6eeff;"">" & objRS("ChangeGroup") & "</TD>" & _
					"<TD>" & strAction & "</TD></TR>"
...
'						strPages2 = strPages2 & "<li class=""page-item""><a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=" & lngTotalRecords - clng(Session("PageCombo")) & """ aria-label=""More""><span aria-hidden=""true"">&hellip;</span><span class=""sr-only"">More</span></a></li>"
'					End If
...
'						strPages2 = strPages2 & "<li class=""page-item""><a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=" & lngTotalRecords - (clng(Session("PageCombo"))*20) & """ aria-label=""More""><span aria-hidden=""true"">&hellip;</span><span class=""sr-only"">More</span></a></li>"
'					End If
'				Else
...
'					End If
					...
					'strPages = strPages & "<a href=""CSTransactionsToNAB.asp?StartingRecord=" & (x * clng(Session("PageCombo"))) & """> " & x + 1 & " </a>"
'					strPages2 = strPages2 & "<li class=""page-item " & strActive & """><a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=" & (x * clng(Session("PageCombo"))) & """>" & x & "</a></li>"
					
					'strPages = strPages & "<a href=""CSTransactionsToNAB.asp?StartingRecord=" & (x * 50) & """> " & x & " </a>"
					'strPages2 = strPages2 & "<li class=""page-item " & strActive & " -" & x & "|" & lngPage & """><a class=""page-link"" href=""CSTransactionsToNAB.asp?Previous&StartingRecord=" & lngStartingRecord -50 & """>" & x & "</a></li>"
'					End If
'				End If
'			Next
...
'	End If
	...
	'If y > 0 Then
	'	Response.Write "<TR><TH colspan=""9"" style=""text-align:center;""><a href=""CSTransactionsToNAB.asp?Previous&StartingRecord=" & lngStartingRecord -50 & """>Previous Page " & strPages & " <a href=""CSTransactionsToNAB.asp?Previous&StartingRecord=" & lngStartingRecord + 50 & """> Next Page</TH></TR>"
	'End If
...
'			"<a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=0"" aria-label=""Previous""><span aria-hidden=""true"">&laquo;</span><span class=""sr-only"">Previous</span></a></li>" & _
'			strPages2 & _
'			"<li class=""page-item"">" & _
'			"<a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=" & lngTotalRecords - clng(Session("PageCombo")) & """ aria-label=""Next""><span aria-hidden=""true"">&raquo;</span><span class=""sr-only"">Next</span>" & _
'			"</a></li></ul></nav></div></div></div>"
			
			'"<a class=""page-link"" href=""CSTransactionsToNAB.asp?StartingRecord=" & lngStartingRecord - clng(Session("PageCombo")) & """ aria-label=""Previous""><span aria-hidden=""true"">&laquo;</span><span class=""sr-only"">Previous</span></a></li>" & _
'	End If
...
End If
	...
	Response.Write "<div class=""btn-group btn-selector"" role=""group"" aria-label=""Basic example"">" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(1) & """ onClick=""self.location.href='CSTransactionsToNAB.asp?ViewButton=All';""><i class=""fa fa-folder""></i> View All</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(3) & """ onClick=""self.location.href='CSTransactionsToNAB.asp?ViewButton=NoChange';""><i class=""fa fa-thumbs-down""></i> View No Change</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(4) & """ onClick=""self.location.href='CSTransactionsToNAB.asp?ViewButton=Change';""><i class=""fa fa-thumbs-up""></i> View Changes</button>" & _
				"<button type=""button"" class=""btn btn-outline-primary " & arrButton(5) & """ onClick=""self.location.href='CSTransactionsToNAB.asp?ViewButton=Cancelled';""><i class=""fa fa-times""></i> View Cancels</button>" & _
				"&nbsp;&nbsp;<button type=""button"" class=""btn btn" & strViewDeleted & "-danger btn"" onClick=""self.location.href='CSTransactionsToNAB.asp?ViewDeleted=1';"" title=""Click to Show Deleted/Removed CS records""><i class=""fa fa-times""></i> " & strDeletedText & "</button>" & _
				"</div>"

				'"<button type=""button"" class=""btn btn-outline-primary " & arrButton(2) & """ onClick=""self.location.href='CSTransactionsToNAB.asp?ViewButton=Processed';""><i class=""fa fa-cogs""></i> View Processed</button>" & _
End Sub
...
'If strCardType = "DTC" Then
	strFileNameStart = GetSystemAdmin("CMFileStart")
'Else
'	strFileNameStart = GetSystemAdmin("CSFileStartDPC")
'End If

'If strCardType = "DTC" Then
	strNextFileNumber = GetSystemAdmin("CMFileNumberTo")
'Else
...
strFileName = strFileNameStart & PadDigits(Right(Year(Now()),4),4) & PadDigits(Month(Now()),2) & PadDigits(Day(Now()),2) & PadDigits(Hour(Now()),2) & PadDigits(Minute(Now()),2) & PadDigits(Second(Now()),2) & ".txt"

intLen = Len(strFileName)
strFileName1 = Left(strFileName,intLen-4)
...
Dim objTextStream
	...
	objCon.Execute "UPDATE tblCAPSNABCM SEt Address1 = REPLACE(Address1,char(9),' ') WHERE charindex(char(9),Address1)>0"
	objCon.Execute "UPDATE tblCAPSNABCM SEt Address2 = REPLACE(Address2,char(9),' ') WHERE charindex(char(9),Address2)>0"
	objCon.Execute "UPDATE tblCAPSNABCM SEt Address3 = REPLACE(Address3,char(9),' ') WHERE charindex(char(9),Address3)>0"
	'objCon.Execute "UPDATE tblCAPSNABCM SEt CardUpdateInd = 'E' WHERE CardUpdateInd Is Null"



	'Open a recordset of all of the NA File records yet to be exported
	strSQL = "SELECT * FROM tblCAPSNABCM WITH(NOLOCK) WHERE (FileSeqNum Is Null OR FileSeqNum = '') AND ([Status] = 'Awaiting Export' Or [Status] = '')"
	objRS.Open strSQL,objCon
		...
		lngFileLoadID = SaveFileLoadID ("CMToNAB",strFileNameStart & strFileDateTime & ".txt", strFilePath,-1,0,0,0,0,0,0,0,strFileDateTime,strNextFileNumber,"Exported",Session("UserID"),"N")
		
			Set objTextStream = objFSO.OpenTextFile(strFilePath, fsoForWriting, True)
			'objTextStream.WriteLine "H CM" & PadSpaceRight(strFileNameStart,34) & strFileDateTimeSec & strNextFileNumber & PadSpaceRight(" ",476)
			objTextStream.WriteLine PadSpaceLeft("H CM" & PadSpaceLeft(strFileName1,34) & PadSpaceLeft(strFileDateTimeSec,14) & strNextFileNumber & "COADOD",700)
			...
			Do Until objRS.EOF
				...
				strError = strError & CheckForNull(objRS("CardNumber"),"Card Number",intRecordCount)
				'strError = strError & CheckForNull(objRS("CardUpdateInd"),"Card Update Ind",intRecordCount)
				strError = strError & CheckForNull(objRS("CardExpiry"),"Card Expiry Date",intRecordCount)
				...
				strError = strError & CheckForNull(objRS("FirstName"),"First Name",intRecordCount)
				strError = strError & CheckForNull(objRS("Surname"),"Surname",intRecordCount)				
				strError = strError & CheckForNull(objRS("EmbossingName"),"Name on Card",intRecordCount)
				strError = strError & CheckForNull(objRS("DateOfBirth"),"DOB",intRecordCount)
				strError = strError & CheckForNull(objRS("SexGender"),"Sex Gender",intRecordCount)
				...
				strError = strError & CheckForNull(objRS("Address4"),"Address 3",intRecordCount)				
				strError = strError & CheckForNull(objRS("Suburb"),"Suburb",intRecordCount)
				strError = strError & CheckForNull(objRS("StateCode"),"State",intRecordCount)
				...
				strError = strError & CheckForNull(objRS("EmailAddress"),"Email",intRecordCount)
				strError = strError & CheckForNull(objRS("CompID"),"Comp ID",intRecordCount)
				strError = strError & CheckForNull(objRS("CardCreditLimit"),"Credit Limit",intRecordCount)				
				...
				Dim strPCOut
				strPCOut = objRS("PostCode")
				If strPCOut = "            " or strPCOut = "" Then	
					strPCOut = ""
				Else
					strPCOut = "00000000" & objRS("PostCode")
				End If
				...
				'UPDATE -- Changed phone number to pad space left instead of pad digits THEN replaced phone numbers with variables above for nulls/empties to have zeroes
					objTextStream.WriteLine "D " & PadSpaceLeft(objRS("EIDNo"),10) & PadSpaceLeft(objRS("CardNumber"),19) & PadSpaceLeft(objRS("CardExpiry"),6) & PadSpaceLeft(objRS("CardStatus"),2) & PadSpaceLeft(objRS("Title"),4) & PadSpaceLeft(objRS("FirstName"),12) & PadSpaceLeft(objRS("MiddleName"),11) & PadSpaceLeft(objRS("Surname"),25) & PadSpaceLeft(objRS("EmbossingName"),18) & PadSpaceLeft(objRS("DateOfBirth"),8) & PadSpaceLeft(objRS("SexGender"),1) & PadSpaceLeft(objRS("Address1"),30) & PadSpaceLeft(objRS("Address2"),30) & PadSpaceLeft(objRS("Address3"),30) & PadSpaceLeft(objRS("Address4"),30) & PadSpaceLeft(objRS("Suburb"),22) &  PadSpaceLeft(objRS("StateCode"),4) & PadSpaceLeft(strPCOut,12) &  PadSpaceLeft(strWorkPhone,16) & PadSpaceLeft(strMobilePhone,16) & PadSpaceLeft(objRS("EmailAddress"),70) & PadSpaceLeft(objRS("CompID"),8) & PadSpaceLeft(objRS("CardCreditLimit"),11) & PadSpaceLeft(objRS("TransLimit"),1) & PadSpaceLeft(objRS("TransLimitCode"),3)  & PadSpaceLeft(objRS("RestrictCash"),1)  & PadSpaceLeft(objRS("CashLimitCode"),3)  & PadSpaceLeft("",295)  
			...
			objTextStream.WriteLine "T " & strRecordCount & PadSpaceRight(" ",692)		
			...
			'If strCardType = "DTC" Then
				Call UpdateFileLoadSummary ("CMToNAB",strNextFileNumber, strFileName, lngFileLoadID)
			'Else
			...
			If strError = "" Then
				Response.Write "<div class=""alert alert-success"" role=""alert"">CM File " & strFileName & " ADDED to the CS file export folder!</div>"
			Else
				Response.Write "<div class=""alert alert-danger"" role=""alert"">CM File " & strFileName & " has errors : " & strError & "</div>"
			End If
	...
	If objFSO.FileExists(strFilePathTo) Then
		...
		For x = 1 to 10
			...
			Else
				...
				Response.Write strFilePathFrom & "<BR>"
				Response.Write strFilePathTo & "<BR>"
...
Dim intRecord
  	...
  	With objCmd
		...
		.CommandTimeout = 900
		.CommandType = 4
		.CommandText = "spCAPSCSFileExportCardNAB"
...
Dim strSQL
	...
	If IsNumeric(lngBatchNumber) Then
		...
		'If strCardType = "DTC" Then
			strSQL = "UPDATE tblCAPSSystemParameters SET [ParameterValue] = '" & lngBatchNumber & "' WHERE [ParameterName] = 'CMFileNumberTo'"
		'Else
	...
	Else
		...
		Response.Write "<div class=""alert alert-danger"" role=""alert"">ERROR! C File Batch Number: " & lngBatchNumber & " is not a number. See System Admin.</div>"
...
'Description:	Loads all Batch Numbers to a list for selecting and searching/filtering
	...
	'If strCardType = "DTC" Then
		objRS.Open "SELECT * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE [FileType] = 'CMToNab' AND [Deleted] = 'N' ORDER By [FileSeqNum] DESC",objCon
	'Else
	'	objRS.Open "SELECT * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE [FileType] = 'CMToNabDPC' AND [Deleted] = 'N' ORDER By [FileSeqNum] DESC",objCon
	'End If
	'objRS.Open "SELECT * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE [FileType] = 'CMToNab' AND [Deleted] = 'N' ORDER By [FileSeqNum] DESC",objCon
	...
	Response.write "<OPTION value=""0"">Select a Batch to View...</OPTION><OPTION value=""0"">CS Transactions To Be Sent Today</OPTION>"
		...
		Do Until objRS.EOF 
			...
			Response.write "<OPTION value=""" & objRS("FileSeqNum") & """>" & objRS("FileSeqNum") & "</OPTION>"
...
'Public Function Get_Batch_Date(strFileLoadID,strCardType)
	...
	'If strCardType = "DTC" Then
		objRS2.Open "SELECT * FROM tblCAPSFileLoad WHERE FileType = 'CMToNab' AND FileSeqNum = '" & strFileLoadID & "'",objCon
	'Else
	'	objRS2.Open "SELECT * FROM tblCAPSFileLoad WHERE FileType = 'CMToNabDPC' AND FileSeqNum = '" & strFileLoadID & "'",objCon
	'End If
