...
If Not IsEmpty(Request.QueryString("Action")) Then
	If Request.QueryString("Action") = "Process" Then
		Call ProcessFile(Request.QueryString("FileSeqNum"),Request.QueryString("FileLoadID"))
	End If
End If
...
{
	document.getElementById('Progress').style.display = "inline";
	self.location="UploadCS2.asp?Action=SaveFileLocal&chkDelete=no"
}

function UploadLocalG()
{
	document.getElementById('Progress').style.display = "inline";
	self.location="UploadCS2.asp?Action=SaveFileLocal&chkDelete=no&ActionType=Service"
}
...
<!-- Modal -->
	<div class="loader" id="ModApp">
        <div class="wrap">
            ...
            <span class="loading-message">Processing...</h6>
        </div>
    </div>
...
<div class="row col-12" style="padding-left:10px; padding-right:10px;">
   ...
   <!--<div class="col-auto text-center">-->
	<button type="button" class="btn btn-primary btn-xs" onclick="UploadLocalG();" Title="Click to Load any existing CS From Diners file in the G Drive Folder"><i class="fa fa-upload"></i> Load CS G</button>&nbsp;
	<button type="button" class="btn btn-outline-secondary btn-xs" onclick="UploadLocal();" Title="Click to Load any existing CS From Diners file in the CAPS Server Folder"><i class="fa fa-upload"></i> Load CS File</button>
  <!--</div>-->
  <!--<div class="col-auto">
...
Dim strStatus
	...
	dteBatchDateFormat = Year(dteBatchDate) & "-" & Right("0" & Month(dteBatchDate), 2) & "-" & Right("0" & Day(dteBatchDate), 2)
		...
		objRS.Open "SELECT TOP 10 * FROM tblCAPSFileLoad WITH(NOLOCK) WHERE Deleted = 'N' AND DateLoaded > '" & dteBatchDate & "' AND FileType = 'CSFromDiners' ORDER BY FileSeqNum DESC",objCon
		'objRS.Open "SELECT TOP 50 * FROM qryCAPSCSFromDinersSummary WHERE DateUpdated > '" & dteBatchDate & "' ORDER BY BatchNo DESC",objCon
		
		    If objRS.EOF Then
		        Response.Write"<table Class=""table"" WIDTH=""100%"" BORDER=""1"" CELLSPACING=""1"" CELLPADDING=""1"">" & _
                "<tr><th colspan=""11"" style=""text-align:left"">There is no CS From Diners data loaded (Since <input class=""DateClick2"" type=""date"" style=""color:red;"" value=""" & dteBatchDateFormat & """ id=""CSDate"" onChange=""DatePickChange();""/>)</th></tr>" 
		        
		    Else
		    
		         Response.Write "<table Class=""table table-bordered mb-0 newd"" cellspacing=""0"" cellpadding=""0"">" & _
                "<tr><th colspan=""12"" style=""text-align:left"">CS From Diners Summary (Since <input class=""DateClick2"" type=""date"" style=""color:red;"" value=""" & dteBatchDateFormat & """ id=""CSDate"" onChange=""DatePickChange();"" />)</th></tr>" & _
		    ...
		    Do until objRS.eof
				...
				If strStatus = "Imported" Then
					strAction = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ModApp"" onclick=""self.location='UploadCS2.asp?Action=Process&FileSeqNum=" & objRS("FileSeqNum") & "&FileLoadID=" & objRS("FileLoadID") & "'""; title=""Click to Process the CS File and update changes to cards in CAPS from the CS File loaded " & objRS("DateLoaded") & """>Process</button>"
				...
				strView = "<button type=""button"" class=""btn btn-outline-secondary btn-xs"" data-toggle=""modal"" data-target=""#ModApp"" onclick=""self.location='CSTransactions.asp?FileLoadID=" & objRS("FileSeqNum") & "'""; title=""Click to view details of the CS File loaded " & objRS("DateLoaded") & """>View</button>"
				
				
				Response.Write "<TR><TD><a href=""UploadCS2.asp?BatchNo=" & objRS("FileSeqNum") & """>" & objRS("FileSeqNum") & "</A></B></TD>" & _
							"<TD style=""text-align:center"">" & objRS("FileDatetime") & "</TD><TD style=""text-align:center"">" & objRS("RecordCount") & "</TD>" & _
							...
							"<TD style=""text-align:center"" Title=""" & objRS("DateLoaded") & """>" & dteDateLoaded & "</TD><TD style=""text-align:center"">" & strView & "</TD></TR>"
    			objRS.Movenext			
		    Loop
...
'			lngFileID = GetFileLoadID("CSFromDiners","",strFileName)
'
'			If lngFileID = "" Then
...
'			End If
			...
			'The CS File From Diners is a text file so run the ReadText procedure
			ReadText filePath,strFileName
...
x = 0 
	...
	Do Until objTextFile.AtEndOfStream
		...
		If x = 1 Then
			...
			strFileSeqNumNew = GetSystemAdmin("CSFromDiners")
			...
			If GetFileLoadID("CSFromDiners",strFileSeqNum,strFileName) = "" Then				
				If clng(strFileSeqNum) <> clng(strFileSeqNumNew) Then
					Response.write "<div class=""content-body""><section id=""basic-alerts""><div class=""alert alert-danger alert-dismissible mb-2"" role=""alert""><button type=""button"" class=""close"" data-dismiss=""alert"" aria-label=""Close"">" & _
						"<span aria-hidden=""true"">&times;</span></button>" & _
						"<div class=""d-flex align-items-center""><i class=""bx bx-error""></i>" & _
						"<span>NOT LOADED! The CS From Diners File Seq Num """ & strFileSeqNumNew & """ is not the correct next Batch No to be loaded.  It should be Seq Num """ & PadDigits(strFileSeqNum,6) & """</span></div></div></div>"
					Exit Sub
				End If
			Else
				'If the checkbox to overwrite is checked then load the data, otherwise do not load
				...
				Else
					Response.write "<div class=""content-body""><section id=""basic-alerts""><div class=""alert alert-danger alert-dismissible mb-2"" role=""alert""><button type=""button"" class=""close"" data-dismiss=""alert"" aria-label=""Close"">" & _
						"<span aria-hidden=""true"">&times;</span></button>" & _
						"<div class=""d-flex align-items-center""><i class=""bx bx-error""></i>" & _
						"<span>NOT LOADED! The CS From Diners File Seq Num """ & strFileSeqNum & """ has already been loaded! </span></div></div></div>"
					Exit Sub
				End If
			End If
	...
	If x > 1 Then
		...
		If Request.QueryString("ActionType")="Service" Then
			...
			'response.write strArchiveFolder & "Imports\CS File\" & strFileName
...
Public Function ProcessFile(strFileSeqNum,lngFileID)
...
Set objFSO = CreateObject("Scripting.FileSystemObject")
	...
	For Each objFile in colFiles
		...
		If intCount < 6 Then
			...
			Response.Write "<TR><TD Title=""" & objFile.Name & """>" & strFile & "</TD></TR>"
		End If
...
On Error Goto 0
	...
	Response.Write"<table Class=""table table-bordered mb-0 newd"" cellspacing=""0"" cellpadding=""0"">" & _
			"<tr><th style=""text-align:left"">G Files To Be Loaded <i class=""fa help-tooltip fa-question-circle"" data-toggle=""tooltip"" title=""Files on the G Drive waiting to be loaded. Click 'Load G' button to Load CS File.""></i></th></tr>"
